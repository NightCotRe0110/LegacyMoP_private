...
void ObjectMgr::LoadWorldVisibleDistance()
{
    ...
    do
    {
        ...
        {
            TC_LOG_INFO("server", ">> Loaded world visible distance type %u error", type);
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u world visible distance in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCreatureLocales()
{
    ...
    TC_LOG_INFO("server", ">> Loaded %lu creature locale strings in %u ms", (unsigned long)_creatureLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadGossipMenuItemsLocales()
{
    ...
    TC_LOG_INFO("server", ">> Loaded %lu gossip_menu_option locale strings in %u ms", (unsigned long)_gossipMenuItemsLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPointOfInterestLocales()
{
    ...
    TC_LOG_INFO("server", ">> Loaded %lu points_of_interest locale strings in %u ms", (unsigned long)_pointOfInterestLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCreatureTemplates()
{
    ...
    //                                            81           82            83         84               85                  86          87          88
                                             " questItem6, movementId, RegenHealth, equipment_id, mechanic_immune_mask, flags_extra, ScriptName, VignetteId "
                                             "FROM creature_template;");

    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 creature template definitions. DB table `creature_template` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server", ">> Loaded %u creature definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCreatureDifficultyStat()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 creature difficulty stat definitions. DB table `creature_difficulty_stat` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server", ">> Loaded %u creature difficulty stat  definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCreatureTemplateAddons()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 creature template addon definitions. DB table `creature_template_addon` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql", "Creature template (Entry: %u) does not exist but has a record in `creature_template_addon`", entry);
            continue;
        }
        ...
        for (Tokenizer::const_iterator itr = tokens.begin(); itr != tokens.end(); ++itr)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Creature (Entry: %u) has wrong spell %u defined in `auras` field in `creature_template_addon`.", entry, uint32(atol(*itr)));
                continue;
            }
        ...
        if (creatureAddon.mount)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Creature (Entry: %u) has invalid displayInfoId (%u) for mount defined in `creature_template_addon`", entry, creatureAddon.mount);
        ...
        {
            TC_LOG_ERROR("sql", "Creature (Entry: %u) has invalid emote (%u) defined in `creature_addon`.", entry, creatureAddon.emote);
    ...
    TC_LOG_INFO("server", ">> Loaded %u creature template addons in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::CheckCreatureTemplate(CreatureTemplate const* cInfo)
{
    ...
    if (!factionTemplate)
        TC_LOG_ERROR("sql", "Creature (Entry: %u) has non-existing faction template (%u).", cInfo->Entry, cInfo->faction);
    ...
    if (cInfo->Modelid1)
    {
        ...
        if (!displayEntry)
        {
            TC_LOG_ERROR("sql", "Creature (Entry: %u) lists non-existing Modelid1 id (%u), this can crash the client.", cInfo->Entry, cInfo->Modelid1);
        ...
        if (!modelInfo)
            TC_LOG_ERROR("sql", "No model data exist for `Modelid1` = %u listed by creature (Entry: %u).", cInfo->Modelid1, cInfo->Entry);
    }
    ...
    if (cInfo->Modelid2)
    {
        ...
        if (!displayEntry)
        {
            TC_LOG_ERROR("sql", "Creature (Entry: %u) lists non-existing Modelid2 id (%u), this can crash the client.", cInfo->Entry, cInfo->Modelid2);
        ...
        if (!modelInfo)
            TC_LOG_ERROR("sql", "No model data exist for `Modelid2` = %u listed by creature (Entry: %u).", cInfo->Modelid2, cInfo->Entry);
    }
    ...
    if (cInfo->Modelid3)
    {
        ...
        if (!displayEntry)
        {
            TC_LOG_ERROR("sql", "Creature (Entry: %u) lists non-existing Modelid3 id (%u), this can crash the client.", cInfo->Entry, cInfo->Modelid3);
        ...
        if (!modelInfo)
            TC_LOG_ERROR("sql", "No model data exist for `Modelid3` = %u listed by creature (Entry: %u).", cInfo->Modelid3, cInfo->Entry);
    }
    ...
    if (cInfo->Modelid4)
    {
        ...
        if (!displayEntry)
        {
            TC_LOG_ERROR("sql", "Creature (Entry: %u) lists non-existing Modelid4 id (%u), this can crash the client.", cInfo->Entry, cInfo->Modelid4);
        ...
        if (!modelInfo)
            TC_LOG_ERROR("sql", "No model data exist for `Modelid4` = %u listed by creature (Entry: %u).", cInfo->Modelid4, cInfo->Entry);
    }

    if (!displayScaleEntry)
        TC_LOG_ERROR("sql", "Creature (Entry: %u) does not have any existing display id in Modelid1/Modelid2/Modelid3/Modelid4.", cInfo->Entry);
    ...
    for (int k = 0; k < MAX_KILL_CREDIT; ++k)
    {
        if (cInfo->KillCredit[k])
        {
            ...
            {
                TC_LOG_ERROR("sql", "Creature (Entry: %u) lists non-existing creature entry %u in `KillCredit%d`.", cInfo->Entry, cInfo->KillCredit[k], k + 1);
    ...
    {
        TC_LOG_ERROR("sql", "Creature (Entry: %u) has invalid unit_class (%u) in creature_template. Set to 1 (UNIT_CLASS_WARRIOR).", cInfo->Entry, cInfo->unit_class);
        const_cast<CreatureTemplate*>(cInfo)->unit_class = UNIT_CLASS_WARRIOR;
    }

    if (cInfo->dmgschool >= MAX_SPELL_SCHOOL)
    {
        TC_LOG_ERROR("sql", "Creature (Entry: %u) has invalid spell school value (%u) in `dmgschool`.", cInfo->Entry, cInfo->dmgschool);
    ...
    if ((cInfo->npcflag & UNIT_NPC_FLAG_TRAINER) && cInfo->trainer_type >= MAX_TRAINER_TYPE)
        TC_LOG_ERROR("sql", "Creature (Entry: %u) has wrong trainer type %u.", cInfo->Entry, cInfo->trainer_type);

    if (cInfo->type && !sCreatureTypeStore.LookupEntry(cInfo->type))
    {
        TC_LOG_ERROR("sql", "Creature (Entry: %u) has invalid creature type (%u) in `type`.", cInfo->Entry, cInfo->type);
    ...
    {
        TC_LOG_ERROR("sql", "Creature (Entry: %u) has invalid creature family (%u) in `family`.", cInfo->Entry, cInfo->family);
        const_cast<CreatureTemplate*>(cInfo)->family = 0;
    }

    if (cInfo->InhabitType <= 0 || cInfo->InhabitType > INHABIT_ANYWHERE)
    {
        TC_LOG_ERROR("sql", "Creature (Entry: %u) has wrong value (%u) in `InhabitType`, creature will not correctly walk/swim/fly.", cInfo->Entry, cInfo->InhabitType);
        const_cast<CreatureTemplate*>(cInfo)->InhabitType = INHABIT_ANYWHERE;
    }

    if (cInfo->HoverHeight < 0.0f)
    {
        TC_LOG_ERROR("sql", "Creature (Entry: %u) has wrong value (%f) in `HoverHeight`", cInfo->Entry, cInfo->HoverHeight);
    ...
    if (cInfo->VehicleId)
    {
        ...
        if (!vehId)
        {
             TC_LOG_ERROR("sql", "Creature (Entry: %u) has a non-existing VehicleId (%u). This *WILL* cause the client to freeze!", cInfo->Entry, cInfo->VehicleId);
    ...
    if (cInfo->PetSpellDataId)
    {
        ...
        if (!spellDataId)
            TC_LOG_ERROR("sql", "Creature (Entry: %u) has non-existing PetSpellDataId (%u).", cInfo->Entry, cInfo->PetSpellDataId);
    }
    ...
    for (uint8 j = 0; j < CREATURE_MAX_SPELLS; ++j)
    {
        if (cInfo->spells[j] && !sSpellMgr->GetSpellInfo(cInfo->spells[j]))
        {
            ...
            TC_LOG_ERROR("sql", "Creature (Entry: %u) has non-existing Spell%d (%u), set to 0.", cInfo->Entry, j+1, cInfo->spells[j]);
    ...
    {
        TC_LOG_ERROR("sql", "Creature (Entry: %u) has wrong movement generator type (%u), ignored and set to IDLE.", cInfo->Entry, cInfo->MovementType);
    ...
    if (cInfo->equipmentId > 0)                          // 0 no equipment
    {
        ...
        {
            TC_LOG_ERROR("sql", "Table `creature_template` lists creature (Entry: %u) with `equipment_id` %u not found in table `creature_equip_template`, set to no equipment.", cInfo->Entry, cInfo->equipmentId);
    ...
    {
        TC_LOG_ERROR("sql", "Table `creature_template` lists creature (Entry: %u) with `exp` %u. Ignored and set to 0.", cInfo->Entry, cInfo->expansion);
        const_cast<CreatureTemplate*>(cInfo)->expansion = 0;
    }

    if (cInfo->expansionUnknown > MAX_CREATURE_BASE_HP)
    {
        TC_LOG_ERROR("sql", "Table `creature_template` lists creature (Entry: %u) with `exp_unk` %u. Ignored and set to 0.", cInfo->Entry, cInfo->expansionUnknown);
        const_cast<CreatureTemplate*>(cInfo)->expansionUnknown = 0;
    }

    if (uint32 badFlags = (cInfo->flags_extra & ~CREATURE_FLAG_EXTRA_DB_ALLOWED))
    {
        TC_LOG_ERROR("sql", "Table `creature_template` lists creature (Entry: %u) with disallowed `flags_extra` %u, removing incorrect flag.", cInfo->Entry, badFlags);
...
void ObjectMgr::LoadCreatureAddons()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 creature addon definitions. DB table `creature_addon` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!creData)
        {
            TC_LOG_ERROR("sql", "Creature (GUID: %u) does not exist but has a record in `creature_addon`", guid);
            continue;
        }
        ...
        if (creData->movementType == WAYPOINT_MOTION_TYPE && !creatureAddon.path_id)
        {
            ...
            TC_LOG_ERROR("sql", "Creature (GUID %u) has movement type set to WAYPOINT_MOTION_TYPE but no path assigned", guid);
        }
        ...
        for (Tokenizer::const_iterator itr = tokens.begin(); itr != tokens.end(); ++itr)
        {
            ...
            if (!AdditionalSpellInfo)
            {
                TC_LOG_ERROR("sql", "Creature (GUID: %u) has wrong spell %u defined in `auras` field in `creature_addon`.", guid, uint32(atol(*itr)));
                continue;
            }
            creatureAddon.auras[i++] = uint32(atol(*itr));
        }
        ...
        if (creatureAddon.mount)
        {
            if (!sCreatureDisplayInfoStore.LookupEntry(creatureAddon.mount))
            {
                TC_LOG_ERROR("sql", "Creature (GUID: %u) has invalid displayInfoId (%u) for mount defined in `creature_addon`", guid, creatureAddon.mount);
                creatureAddon.mount = 0;
            }
        }
        ...
        if (!sEmotesStore.LookupEntry(creatureAddon.emote))
        {
            TC_LOG_ERROR("sql", "Creature (GUID: %u) has invalid emote (%u) defined in `creature_addon`.", guid, creatureAddon.emote);
            creatureAddon.emote = 0;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u creature addons in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadEquipmentTemplates()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 creature equipment templates. DB table `creature_equip_template` is empty!");
        return;
    }
    ...
    do
    {
        ...
        for (uint8 i = 0; i < MAX_EQUIPMENT_ITEMS; ++i)
        {
            ...
            if (!dbcItem)
            {
                TC_LOG_ERROR("sql", "Unknown item (entry=%u) in creature_equip_template.itemEntry%u for entry = %u, forced to 0.",
            ...
            {
                TC_LOG_ERROR("sql", "Item (entry=%u) in creature_equip_template.itemEntry%u for entry = %u is not equipable in a hand, forced to 0.",
                    equipmentInfo.ItemEntry[i], i+1, entry);
                equipmentInfo.ItemEntry[i] = 0;
            }
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u equipment templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
CreatureModelInfo const* ObjectMgr::GetCreatureModelRandomGender(uint32* displayID)
{
    ...
    if (modelInfo->modelid_other_gender != 0 && urand(0, 1) == 0)
    {
        ...
        if (!minfo_tmp)
            TC_LOG_ERROR("sql", "Model (Entry: %u) has modelid_other_gender %u not found in table `creature_model_info`. ", *displayID, modelInfo->modelid_other_gender);
        else
        {
            // Model ID changed
...
void ObjectMgr::LoadCreatureModelInfo()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 creature model definitions. DB table `creature_model_info` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql", "Table `creature_model_info` has model for not existed display id (%u).", modelId);
            continue;
        }
        ...
        {
            TC_LOG_ERROR("sql", "Table `creature_model_info` has wrong gender (%u) for display id (%u).", uint32(modelInfo.gender), modelId);
            modelInfo.gender = GENDER_MALE;
        }

        if (modelInfo.modelid_other_gender && !sCreatureDisplayInfoStore.LookupEntry(modelInfo.modelid_other_gender))
        {
            TC_LOG_ERROR("sql", "Table `creature_model_info` has not existed alt.gender model (%u) for existed display id (%u).", modelInfo.modelid_other_gender, modelId);
            modelInfo.modelid_other_gender = 0;
        }

        if (modelInfo.hostileId && !sCreatureDisplayInfoStore.LookupEntry(modelInfo.hostileId))
        {
            TC_LOG_ERROR("sql", "Table `creature_model_info` has not existed alt.hostileId model (%u) for existed display id (%u).", modelInfo.hostileId, modelId);
    ...
    TC_LOG_INFO("server", ">> Loaded %u creature model based info in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadLinkedRespawn()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 linked respawns. DB table `linked_respawn` is empty.");
    ...
    do
    {
        ...
        switch (linkType)
        {
            case CREATURE_TO_CREATURE:
            {
                ...
                if (!slave)
                {
                    TC_LOG_ERROR("sql", "Couldn't get creature data for GUIDLow %u", guidLow);
                    error = true;
                    break;
                }
                ...
                if (!master)
                {
                    TC_LOG_ERROR("sql", "Couldn't get creature data for GUIDLow %u", linkedGuidLow);
                    error = true;
                    break;
                }
                ...
                {
                    TC_LOG_ERROR("sql", "Creature '%u' linking to '%u' on an unpermitted map.", guidLow, linkedGuidLow);
                    error = true;
                    break;
                }
                ...
                {
                    TC_LOG_ERROR("sql", "LinkedRespawn: Creature '%u' linking to '%u' with not corresponding spawnMask", guidLow, linkedGuidLow);
                    error = true;
                    break;
                }
            ...
            case CREATURE_TO_GO:
            {
                ...
                if (!master)
                {
                    TC_LOG_ERROR("sql", "Couldn't get gameobject data for GUIDLow %u", linkedGuidLow);
                    error = true;
                    break;
                }
            ...
            case GO_TO_GO:
            {
                ...
                if (!slave)
                {
                    TC_LOG_ERROR("sql", "Couldn't get gameobject data for GUIDLow %u", guidLow);
                    error = true;
                    break;
                }
    ...
    TC_LOG_INFO("server", ">> Loaded " UI64FMTD " linked respawns in %u ms", uint64(_linkedRespawnStore.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCreatures()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 creatures. DB table `creature` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!cInfo)
        {
            TC_LOG_ERROR("sql", "Table `creature` has creature (GUID: %u) with non existing creature entry %u, skipped.", guid, entry);
            continue;
        }
        ...
        if (!mapEntry)
        {
            TC_LOG_ERROR("sql", "Table `creature` have creature (GUID: %u) that spawned at not existed map (Id: %u), skipped.", guid, data.mapid);
            continue;
        }

        if (data.spawnMask & ~spawnMasks[data.mapid])
        {
            TC_LOG_ERROR("sql", "Table `creature` have creature (GUID: %u) that have wrong spawn mask %u including not supported difficulty modes for map (Id: %u) spawnMasks[data.mapid]: %u.", guid, data.spawnMask, data.mapid, spawnMasks[data.mapid]);
        ...
        if (data.equipmentId > 0)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Table `creature` have creature (Entry: %u) with equipment_id %u not found in table `creature_equip_template`, set to no equipment.", data.id, data.equipmentId);
                data.equipmentId = -1;
            }
        }
        ...
        if (cInfo->flags_extra & CREATURE_FLAG_EXTRA_INSTANCE_BIND)
        {
            if (!mapEntry || !mapEntry->IsDungeon())
                TC_LOG_ERROR("sql", "Table `creature` have creature (GUID: %u Entry: %u) with `creature_template`.`flags_extra` including CREATURE_FLAG_EXTRA_INSTANCE_BIND but creature are not in instance.", guid, data.id);
        }

        if (data.spawndist < 0.0f)
        {
            TC_LOG_ERROR("sql", "Table `creature` have creature (GUID: %u Entry: %u) with `spawndist`< 0, set to 0.", guid, data.id);
            data.spawndist = 0.0f;
        }
        else if (data.movementType == RANDOM_MOTION_TYPE)
        {
            if (data.spawndist == 0.0f)
            {
                TC_LOG_ERROR("sql", "Table `creature` have creature (GUID: %u Entry: %u) with `MovementType`=1 (random movement) but with `spawndist`=0, replace by idle movement type (0).", guid, data.id);
        ...
        else if (data.movementType == IDLE_MOTION_TYPE)
        {
            if (data.spawndist != 0.0f)
            {
                TC_LOG_ERROR("sql", "Table `creature` have creature (GUID: %u Entry: %u) with `MovementType`=0 (idle) have `spawndist`<>0, set to 0.", guid, data.id);
                data.spawndist = 0.0f;
            }
        }
        ...
        if (data.phaseMask == 0)
        {
            TC_LOG_ERROR("sql", "Table `creature` have creature (GUID: %u Entry: %u) with `phaseMask`=0 (not visible for anyone), set to 1.", guid, data.id);
            data.phaseMask = 1;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u creatures in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCreatureAIInstance()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 creature AI instance. DB table `creature_ai_instance` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server", ">> Loaded %u creature AI instance in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 creature AI instance door. DB table `creature_ai_instance_door` is empty.");
        return;
    }
...
void ObjectMgr::LoadPersonalLootTemplate()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 personal loot. DB table `personal_loot_template` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server", ">> Loaded %u personal loot in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
uint32 ObjectMgr::AddGOData(uint32 entry, uint32 mapId, float x, float y, float z, float o, uint32 spawntimedelay, float rotation0, float rotation1, float rotation2, float rotation3)
{
    ...
    if (!map->Instanceable() && map->IsGridLoaded(x, y))
    {
        ...
        {
            TC_LOG_ERROR("server", "AddGOData: cannot add gameobject entry %u to map", entry);
            delete go;
            return 0;
        }
    }
...
bool ObjectMgr::MoveCreData(uint32 guid, uint32 mapId, Position pos)
{
    ...
    if (Map* map = sMapMgr->CreateBaseMap(mapId))
    {
        ...
        if (!map->Instanceable() && map->IsGridLoaded(data.posX, data.posY))
        {
            ...
            {
                TC_LOG_ERROR("server", "AddCreature: cannot add creature entry %u to map", guid);
                delete creature;
                return false;
            }
        }
...
uint32 ObjectMgr::AddCreData(uint32 entry, uint32 /*team*/, uint32 mapId, float x, float y, float z, float o, uint32 spawntimedelay)
{
    ...
    if (Map* map = sMapMgr->CreateBaseMap(mapId))
    {
        ...
        if (!map->Instanceable() && !map->IsRemovalGrid(x, y))
        {
            ...
            if (!creature->LoadCreatureFromDB(guid, map))
            {
                TC_LOG_ERROR("server", "AddCreature: cannot add creature entry %u to map", entry);
                delete creature;
                return 0;
            }
        }
...
void ObjectMgr::LoadGameobjects()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 gameobjects. DB table `gameobject` is empty.");
    ...
    do
    {
        ...
        if (!gInfo)
        {
            TC_LOG_ERROR("sql", "Table `gameobject` has gameobject (GUID: %u) with non existing gameobject entry %u, skipped.", guid, entry);
            continue;
        }
        ...
        if (!gInfo->displayId)
        {
            switch (gInfo->type)
            {
                ...
                case GAMEOBJECT_TYPE_SPELL_FOCUS:
                    break;
                default:
                    TC_LOG_ERROR("sql", "Gameobject (GUID: %u Entry %u GoType: %u) doesn't have a displayId (%u), not loaded.", guid, entry, gInfo->type, gInfo->displayId);
                    break;
            }
        }
        ...
        {
            TC_LOG_ERROR("sql", "Gameobject (GUID: %u Entry %u GoType: %u) has an invalid displayId (%u), not loaded.", guid, entry, gInfo->type, gInfo->displayId);
            continue;
        }
        ...
        if (!mapEntry)
        {
            TC_LOG_ERROR("sql", "Table `gameobject` has gameobject (GUID: %u Entry: %u) spawned on a non-existed map (Id: %u), skip", guid, data.id, data.mapid);
            continue;
        }
        ...
        {
            TC_LOG_ERROR("sql", "Table `gameobject` has gameobject (GUID: %u Entry: %u) with `spawntimesecs` (0) value, but the gameobejct is marked as despawnable at action.", guid, data.id);
        }
        ...
        {
            TC_LOG_ERROR("sql", "Table `gameobject` has gameobject (GUID: %u Entry: %u) with invalid `state` (%u) value, skip", guid, data.id, go_state);
            continue;
        }
        ...
        if (data.spawnMask & ~spawnMasks[data.mapid])
        {
            TC_LOG_ERROR("sql", "Table `gameobject` has gameobject (GUID: %u Entry: %u) that has wrong spawn mask %u including not supported difficulty modes for map (Id: %u), skip", guid, data.id, data.spawnMask, data.mapid);
        ...
        {
            TC_LOG_ERROR("sql", "Table `gameobject` has gameobject (GUID: %u Entry: %u) with invalid rotation2 (%f) value, skip", guid, data.id, data.rotation2);
            continue;
        }

        if (data.rotation3 < -1.0f || data.rotation3 > 1.0f)
        {
            TC_LOG_ERROR("sql", "Table `gameobject` has gameobject (GUID: %u Entry: %u) with invalid rotation3 (%f) value, skip", guid, data.id, data.rotation3);
            continue;
        }

        if (data.phaseMask == 0)
        {
            TC_LOG_ERROR("sql", "Table `gameobject` has gameobject (GUID: %u Entry: %u) with `phaseMask`=0 (not visible for anyone), set to 1.", guid, data.id);
            data.phaseMask = 1;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %lu gameobjects in %u ms", (unsigned long)_gameObjectDataStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadItemLocales()
{
    ...
    TC_LOG_INFO("server", ">> Loaded %lu Item locale strings in %u ms", (unsigned long)_itemLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadItemTemplates()
{
    ...
    for (std::set<uint32>::const_iterator itr = notFoundOutfit.begin(); itr != notFoundOutfit.end(); ++itr)
        TC_LOG_ERROR("sql", "Item (Entry: %u) does not exist in `item_template` but is referenced in `CharStartOutfit.dbc`", *itr);

    TC_LOG_INFO("server", ">> Loaded %u item templates from Item-sparse.db2 and %u from database in %u ms", sparseCount, dbCount, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadItemTemplateAddon()
{
    ...
    if (result)
    {
        do
        {
            ...
            {
                TC_LOG_ERROR("sql", "Item %u specified in `item_template_addon` does not exist, skipped.", itemId);
                continue;
            }
            ...
            {
                TC_LOG_ERROR("sql", "Minimum money loot specified in `item_template_addon` for item %u was greater than maximum amount, swapping.", itemId);
    ...
    TC_LOG_INFO("server", ">> Loaded %u item addon templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadItemScriptNames()
{
    ...
    if (result)
    {
        do
        {
            ...
            if (!GetItemTemplate(itemId))
            {
                TC_LOG_ERROR("sql", "Item %u specified in `item_script_names` does not exist, skipped.", itemId);
                continue;
            }
    ...
    TC_LOG_INFO("server", ">> Loaded %u item script names in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadVehicleTemplateAccessories()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 vehicle template accessories. DB table `vehicle_template_accessory` is empty.");
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql", "Table `vehicle_template_accessory`: creature template entry %u does not exist.", uiEntryOrAura);
            continue;
        }

        if (uiEntryOrAura < 0 && !sSpellMgr->GetSpellInfo(uiEntryOrAura * -1))
        {
            TC_LOG_ERROR("sql", "Table `vehicle_template_accessory`: aura %u does not exist.", uiEntryOrAura);
            continue;
        }

        if (!sObjectMgr->GetCreatureTemplate(uiAccessory))
        {
            TC_LOG_ERROR("sql", "Table `vehicle_template_accessory`: Accessory %u does not exist.", uiAccessory);
            continue;
        }

        if (uiEntryOrAura > 0 && _spellClickInfoStore.find(uiEntryOrAura) == _spellClickInfoStore.end())
        {
            TC_LOG_ERROR("sql", "Table `vehicle_template_accessory`: creature template entry %u has no data in npc_spellclick_spells", uiEntryOrAura);
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u Vehicle Template Accessories in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadVehicleAccessories()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 Vehicle Accessories in %u ms", GetMSTimeDiffToNow(oldMSTime));
        return;
    }
    ...
    do
    {
        ...
        if (!sObjectMgr->GetCreatureTemplate(uiAccessory))
        {
            TC_LOG_ERROR("sql", "Table `vehicle_accessory`: Accessory %u does not exist.", uiAccessory);
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u Vehicle Accessories in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPetStats()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 pet stats. DB table `pet_stats` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!sObjectMgr->GetCreatureTemplate(entry))
        {
            TC_LOG_ERROR("sql", "Wrong creature id %u in `pet_stats` table, ignoring.", entry);
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u pet stats definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::PlayerCreateInfoAddItemHelper(uint32 race_, uint32 class_, uint32 itemId, int32 count)
{
    ...
    {
        if (count < -1)
            TC_LOG_ERROR("sql", "Invalid count %i specified on item %u be removed from original player create info (use -1)!", count, itemId);
        ...
        for (uint32 i = 1; i < sCharStartOutfitStore.GetNumRows(); ++i)
        {
            if (CharStartOutfitEntry const* entry = sCharStartOutfitStore.LookupEntry(i))
            {
                if (entry->RaceClassGender == RaceClass || entry->RaceClassGender == (RaceClass | (1 << 16)))
                {
                    ...
                    if (!found)
                        TC_LOG_ERROR("sql", "Item %u specified to be removed from original create info not found in dbc!", itemId);
...
void ObjectMgr::LoadPlayerInfo()
{
    // Load playercreate
    {
        ...
        if (!result)
        {
            ...
            TC_LOG_ERROR("server", ">> Loaded 0 player create definitions. DB table `playercreateinfo` is empty.");
            exit(1);
        }
        else
        {
            ...
            do
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Wrong race %u in `playercreateinfo` table, ignoring.", current_race);
                    continue;
                }
                ...
                {
                    TC_LOG_ERROR("sql", "Wrong class %u in `playercreateinfo` table, ignoring.", current_class);
                    continue;
                }
                ...
                {
                    TC_LOG_ERROR("sql", "Wrong home position for class %u race %u pair in `playercreateinfo` table, ignoring.", current_class, current_race);
                    continue;
                }

                if (sMapStore.LookupEntry(mapId)->Instanceable())
                {
                    TC_LOG_ERROR("sql", "Home position in instanceable map for class %u race %u pair in `playercreateinfo` table, ignoring.", current_class, current_race);
                    continue;
                }
            ...
            TC_LOG_INFO("server", ">> Loaded %u player create definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // Load playercreate items
    TC_LOG_INFO("server", "Loading Player Create Items Data...");
    {
        uint32 oldMSTime = getMSTime();
        //                                                0     1      2       3
        ...
        if (!result)
        {
            TC_LOG_INFO("server", ">> Loaded 0 custom player create items. DB table `playercreateinfo_item` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            do
            {
                ...
                if (current_race >= MAX_RACES)
                {
                    TC_LOG_ERROR("sql", "Wrong race %u in `playercreateinfo_item` table, ignoring.", current_race);
                    continue;
                }
                ...
                if (current_class >= MAX_CLASSES)
                {
                    TC_LOG_ERROR("sql", "Wrong class %u in `playercreateinfo_item` table, ignoring.", current_class);
                    continue;
                }
                ...
                if (!GetItemTemplate(item_id))
                {
                    TC_LOG_ERROR("sql", "Item id %u (race %u class %u) in `playercreateinfo_item` table but not listed in `item_template`, ignoring.", item_id, current_race, current_class);
                    continue;
                }
                ...
                if (!amount)
                {
                    TC_LOG_ERROR("sql", "Item id %u (class %u race %u) have amount == 0 in `playercreateinfo_item` table, ignoring.", item_id, current_race, current_class);
                    continue;
                }
            ...
            TC_LOG_INFO("server", ">> Loaded %u custom player create items in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // Load playercreate spells
    TC_LOG_INFO("server", "Loading Player Create Spell Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_ERROR("server", ">> Loaded 0 player create spells. DB table `%s` is empty.", sWorld->getBoolConfig(CONFIG_START_ALL_SPELLS) ? "playercreateinfo_spell_custom" : "playercreateinfo_spell");
        ...
        else
        {
            ...
            do
            {
                ...
                if (current_race >= MAX_RACES)
                {
                    TC_LOG_ERROR("sql", "Wrong race %u in `playercreateinfo_spell` table, ignoring.", current_race);
                    continue;
                }
                ...
                if (current_class >= MAX_CLASSES)
                {
                    TC_LOG_ERROR("sql", "Wrong class %u in `playercreateinfo_spell` table, ignoring.", current_class);
                    continue;
                }
            ...
            TC_LOG_INFO("server", ">> Loaded %u player create spells in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // Load playercreate actions
    TC_LOG_INFO("server", "Loading Player Create Action Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_ERROR("server", ">> Loaded 0 player create actions. DB table `playercreateinfo_action` is empty.");
        ...
        else
        {
            ...
            do
            {
                ...
                if (current_race >= MAX_RACES)
                {
                    TC_LOG_ERROR("sql", "Wrong race %u in `playercreateinfo_action` table, ignoring.", current_race);
                    continue;
                }
                ...
                if (current_class >= MAX_CLASSES)
                {
                    TC_LOG_ERROR("sql", "Wrong class %u in `playercreateinfo_action` table, ignoring.", current_class);
                    continue;
                }
            ...
            TC_LOG_INFO("server", ">> Loaded %u player create actions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // Loading levels data (class/race dependent)
    TC_LOG_INFO("server", "Loading Player Create Level Stats Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_ERROR("server", ">> Loaded 0 level stats definitions. DB table `player_levelstats` is empty.");
        ...
        do
        {
            ...
            if (current_race >= MAX_RACES)
            {
                TC_LOG_ERROR("sql", "Wrong race %u in `player_levelstats` table, ignoring.", current_race);
                continue;
            }
            ...
            if (current_class >= MAX_CLASSES)
            {
                TC_LOG_ERROR("sql", "Wrong class %u in `player_levelstats` table, ignoring.", current_class);
                continue;
            }
            ...
            if (current_level > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                if (current_level > STRONG_MAX_LEVEL)        // hardcoded level maximum
                    TC_LOG_ERROR("sql", "Wrong (> %u) level %u in `player_levelstats` table, ignoring.", STRONG_MAX_LEVEL, current_level);
                else
                {
                    TC_LOG_INFO("server", "Unused (> MaxPlayerLevel in worldserver.conf) level %u in `player_levelstats` table, ignoring.", current_level);
        ...
        for (int race = 0; race < MAX_RACES; ++race)
        {
            ...
            for (int class_ = 0; class_ < MAX_CLASSES; ++class_)
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Race %i Class %i Level 1 does not have stats data!", race, class_);
                    exit(1);
                }
                ...
                for (uint8 level = 1; level < sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL); ++level)
                {
                    ...
                    {
                        TC_LOG_ERROR("sql", "Race %i Class %i Level %i does not have stats data. Using stats data of level %i.", race, class_, level+1, level);
        ...
        TC_LOG_INFO("server", ">> Loaded %u level stats definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }

    // Loading xp per level data
    TC_LOG_INFO("server", "Loading Player Create XP Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_ERROR("server", ">> Loaded 0 xp for level definitions. DB table `player_xp_for_level` is empty.");
        ...
        do
        {
            ...
            {
                if (current_level > STRONG_MAX_LEVEL)        // hardcoded level maximum
                    TC_LOG_ERROR("sql", "Wrong (> %u) level %u in `player_xp_for_level` table, ignoring.", STRONG_MAX_LEVEL, current_level);
                else
                {
                    TC_LOG_INFO("server", "Unused (> MaxPlayerLevel in worldserver.conf) level %u in `player_xp_for_levels` table, ignoring.", current_level);
                    ++count;                                // make result loading percent "expected" correct in case disabled detail mode for example.
                }
                continue;
            }
        ...
        for (uint8 level = 1; level < sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL); ++level)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Level %i does not have XP for level data. Using data of level [%i] + 100.", level+1, level);
                _playerXPperLevel[level] = _playerXPperLevel[level-1]+100;
            }
        }

        TC_LOG_INFO("server", ">> Loaded %u xp for level definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
}
...
void ObjectMgr::GetPlayerClassLevelInfo(uint32 class_, uint8 level, uint32& baseHP, uint32& baseMana) const
{
    ...
    if (!hp || !mp)
    {
        TC_LOG_ERROR("server", "Tried to get non-existant Class-Level combination data for base hp/mp. Class %u Level %u", class_, level);
        return;
    }
...
void ObjectMgr::LoadQuests()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 quests definitions. DB table `quest_template` is empty.");
    ...
    for (QuestMap::iterator iter = _questTemplates.begin(); iter != _questTemplates.end(); ++iter)
    {
        ...
        if (qinfo->GetQuestMethod() >= 3)
            TC_LOG_ERROR("sql", "Quest %u has `Method` = %u, expected values are 0, 1 or 2.", qinfo->GetQuestId(), qinfo->GetQuestMethod());

        if (qinfo->SpecialFlags & ~QUEST_SPECIAL_FLAGS_DB_ALLOWED)
        {
            TC_LOG_ERROR("sql", "Quest %u has `SpecialFlags` = %u > max allowed value. Correct `SpecialFlags` to value <= %u",
        ...
        {
            TC_LOG_ERROR("sql", "Weekly Quest %u is marked as daily quest in `Flags`, removed daily flag.", qinfo->GetQuestId());
        ...
        if (qinfo->Flags & QUEST_FLAGS_DAILY)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Daily Quest %u not marked as repeatable in `SpecialFlags`, added.", qinfo->GetQuestId());
        ...
        if (qinfo->Flags & QUEST_FLAGS_WEEKLY)
        {
            if (!(qinfo->SpecialFlags & QUEST_SPECIAL_FLAGS_REPEATABLE))
            {
                TC_LOG_ERROR("sql", "Weekly Quest %u not marked as repeatable in `SpecialFlags`, added.", qinfo->GetQuestId());
                qinfo->SpecialFlags |= QUEST_SPECIAL_FLAGS_REPEATABLE;
            }
        }
        ...
        if (qinfo->Flags & QUEST_FLAGS_AUTO_REWARDED)
        {
            ...
            for (int j = 1; j < QUEST_REWARD_CHOICES_COUNT; ++j )
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Quest %u has `RewardChoiceItemId%d` = %u but item from `RewardChoiceItemId%d` can't be rewarded with quest flag QUEST_FLAGS_AUTO_REWARDED.",
        ...
        {
            TC_LOG_ERROR("sql", "Quest %u should be disabled because `MinLevel` = %i", qinfo->GetQuestId(), int32(qinfo->MinLevel));
        ...
        if (qinfo->ZoneOrSort < 0)
        {
            ...
            if (!qSort)
            {
                TC_LOG_ERROR("sql", "Quest %u has `ZoneOrSort` = %i (sort case) but quest sort with this id does not exist.",
                    qinfo->GetQuestId(), qinfo->ZoneOrSort);
            ...
            if (uint32 skill_id = SkillByQuestSort(-int32(qinfo->ZoneOrSort)))
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Quest %u has `ZoneOrSort` = %i but `RequiredSkillId` does not have a corresponding value (%d).",
        ...
        if (qinfo->RequiredClasses)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u does not contain any playable classes in `RequiredClasses` (%u), value set to 0 (all classes).", qinfo->GetQuestId(), qinfo->RequiredClasses);
        ...
        if (qinfo->RequiredRaces)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u does not contain any playable races in `RequiredRaces` (%u), value set to 0 (all races).", qinfo->GetQuestId(), qinfo->RequiredRaces);
        ...
        if (qinfo->RequiredSkillId)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RequiredSkillId` = %u but this skill does not exist",
        ...
        if (qinfo->RequiredSkillPoints)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RequiredSkillPoints` = %u but max possible skill is %u, quest can't be done.",
        ...
        {
            TC_LOG_ERROR("sql", "Quest %u has `RequiredFactionId2` = %u but faction template %u does not exist, quest can't be done.",
        ...
        {
            TC_LOG_ERROR("sql", "Quest %u has `RequiredFactionId1` = %u but faction template %u does not exist, quest can't be done.",
        ...
        {
            TC_LOG_ERROR("sql", "Quest %u has `RequiredMinRepFaction` = %u but faction template %u does not exist, quest can't be done.",
        ...
        {
            TC_LOG_ERROR("sql", "Quest %u has `RequiredMaxRepFaction` = %u but faction template %u does not exist, quest can't be done.",
                qinfo->GetQuestId(), qinfo->RequiredMaxRepFaction, qinfo->RequiredMaxRepFaction);
            // no changes, quest can't be done for this requirement
        }

        if (qinfo->RequiredMinRepValue && qinfo->RequiredMinRepValue > 42999)
        {
            TC_LOG_ERROR("sql", "Quest %u has `RequiredMinRepValue` = %d but max reputation is %u, quest can't be done.",
                qinfo->GetQuestId(), qinfo->RequiredMinRepValue, 42999);
            // no changes, quest can't be done for this requirement
        }

        if (qinfo->RequiredMinRepValue && qinfo->RequiredMaxRepValue && qinfo->RequiredMaxRepValue <= qinfo->RequiredMinRepValue)
        {
            TC_LOG_ERROR("sql", "Quest %u has `RequiredMaxRepValue` = %d and `RequiredMinRepValue` = %d, quest can't be done.",
        ...
        {
            TC_LOG_ERROR("sql", "Quest %u has `RequiredFactionValue1` = %d but `RequiredFactionId1` is 0, value has no effect",
        ...
        {
            TC_LOG_ERROR("sql", "Quest %u has `RequiredFactionValue2` = %d but `RequiredFactionId2` is 0, value has no effect",
        ...
        {
            TC_LOG_ERROR("sql", "Quest %u has `RequiredMinRepValue` = %d but `RequiredMinRepFaction` is 0, value has no effect",
        ...
        {
            TC_LOG_ERROR("sql", "Quest %u has `RequiredMaxRepValue` = %d but `RequiredMaxRepFaction` is 0, value has no effect",
        ...
        {
            TC_LOG_ERROR("sql", "Quest %u has `RewardTitleId` = %u but CharTitle Id %u does not exist, quest can't be rewarded with title.",
        ...
        if (qinfo->SourceItemId)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `SourceItemId` = %u but item with entry %u does not exist, quest can't be done.",
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `SourceItemId` = %u but `SourceItemIdCount` = 0, set to 1 but need fix in DB.",
        ...
        {
            TC_LOG_ERROR("sql", "Quest %u has `SourceItemId` = 0 but `SourceItemIdCount` = %u, useless value.",
        ...
        if (qinfo->SourceSpellid)
        {
            ...
            if (!spellInfo)
            {
                TC_LOG_ERROR("sql", "Quest %u has `SourceSpellid` = %u but spell %u doesn't exist, quest can't be done.",
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `SourceSpellid` = %u but spell %u is broken, quest can't be done.",
                    qinfo->GetQuestId(), qinfo->SourceSpellid, qinfo->SourceSpellid);
                qinfo->SourceSpellid = 0;                        // quest can't be done for this requirement
            }
        }
        ...
        for (uint8 j = 0; j < QUEST_ITEM_OBJECTIVES_COUNT; ++j)
        {
            ...
            if (id)
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Quest %u has `RequiredItemId%d` = %u but `RequiredItemCount%d` = 0, quest can't be done.",
                        qinfo->GetQuestId(), j+1, id, j+1);
                    // no changes, quest can't be done for this requirement
                }
                ...
                {
                    TC_LOG_ERROR("sql", "Quest %u has `RequiredItemId%d` = %u but item with entry %u does not exist, quest can't be done.",
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RequiredItemId%d` = 0 but `RequiredItemCount%d` = %u, quest can't be done.",
        ...
        for (uint8 j = 0; j < QUEST_SOURCE_ITEM_IDS_COUNT; ++j)
        {
            ...
            if (id)
            {
                if (!sObjectMgr->GetItemTemplate(id))
                {
                    TC_LOG_ERROR("sql", "Quest %u has `RequiredSourceItemId%d` = %u but item with entry %u does not exist, quest can't be done.",
                        qinfo->GetQuestId(), j+1, id, id);
                    // no changes, quest can't be done for this requirement
                }
            }
            else
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Quest %u has `RequiredSourceItemId%d` = 0 but `RequiredSourceItemCount%d` = %u.",
        ...
        for (uint8 j = 0; j < QUEST_SOURCE_ITEM_IDS_COUNT; ++j)
        {
            ...
            if (id)
            {
                ...
                if (!spellInfo)
                {
                    TC_LOG_ERROR("sql", "Quest %u has `ReqSpellCast%d` = %u but spell %u does not exist, quest can't be done.",
                        qinfo->GetQuestId(), j+1, id, id);
                    continue;
                }
                ...
                if (!qinfo->RequiredNpcOrGo[j])
                {
                    ...
                    if (found)
                    {
                        ...
                        {
                            TC_LOG_ERROR("sql", "Spell (id: %u) have SPELL_EFFECT_QUEST_COMPLETE or SPELL_EFFECT_SEND_EVENT for quest %u and RequiredNpcOrGo%d = 0, but quest not have flag QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT. Quest flags or RequiredNpcOrGo%d must be fixed, quest modified to enable objective.", spellInfo->Id, qinfo->Id, j+1, j+1);
                    ...
                    {
                        TC_LOG_ERROR("sql", "Quest %u has `ReqSpellCast%d` = %u and RequiredNpcOrGo%d = 0 but spell %u does not have SPELL_EFFECT_QUEST_COMPLETE or SPELL_EFFECT_SEND_EVENT effect for this quest, quest can't be done.",
        ...
        for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RequiredNpcOrGo%d` = %i but gameobject %u does not exist, quest can't be done.",
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RequiredNpcOrGo%d` = %i but creature with entry %u does not exist, quest can't be done.",
                    qinfo->GetQuestId(), j+1, id, uint32(id));
                qinfo->RequiredNpcOrGo[j] = 0;            // quest can't be done for this requirement
            }
            ...
            if (id)
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Quest %u has `RequiredNpcOrGo%d` = %u but `RequiredNpcOrGoCount%d` = 0, quest can't be done.",
                        qinfo->GetQuestId(), j+1, id, j+1);
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RequiredNpcOrGo%d` = 0 but `RequiredNpcOrGoCount%d` = %u.",
        ...
        for (uint8 j = 0; j < QUEST_REWARD_CHOICES_COUNT; ++j)
        {
            ...
            if (id)
            {
                if (!sObjectMgr->GetItemTemplate(id))
                {
                    TC_LOG_ERROR("sql", "Quest %u has `RewardChoiceItemId%d` = %u but item with entry %u does not exist, quest will not reward this item.",
                        qinfo->GetQuestId(), j+1, id, id);
                ...
                {
                    TC_LOG_ERROR("sql", "Quest %u has `RewardChoiceItemId%d` = %u but `RewardChoiceItemCount%d` = 0, quest can't be done.",
                        qinfo->GetQuestId(), j+1, id, j+1);
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RewardChoiceItemId%d` = 0 but `RewardChoiceItemCount%d` = %u.",
        ...
        for (uint8 j = 0; j < QUEST_REWARDS_COUNT; ++j)
        {
            ...
            if (id)
            {
                if (!sObjectMgr->GetItemTemplate(id))
                {
                    TC_LOG_ERROR("sql", "Quest %u has `RewardItemId%d` = %u but item with entry %u does not exist, quest will not reward this item.",
                        qinfo->GetQuestId(), j+1, id, id);
                ...
                {
                    TC_LOG_ERROR("sql", "Quest %u has `RewardItemId%d` = %u but `RewardItemIdCount%d` = 0, quest will not reward this item.",
                        qinfo->GetQuestId(), j+1, id, j+1);
                    // no changes
                }
            }
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RewardItemId%d` = 0 but `RewardItemIdCount%d` = %u.",
        ...
        for (uint8 j = 0; j < QUEST_REPUTATIONS_COUNT; ++j)
        {
            if (qinfo->RewardFactionId[j])
            {
                ...
                {
               TC_LOG_ERROR("sql", "Quest %u has RewardFactionValueId%d = %i. That is outside the range of valid values (-9 to 9).", qinfo->GetQuestId(), j+1, qinfo->RewardFactionValueId[j]);
                }
                if (!sFactionStore.LookupEntry(qinfo->RewardFactionId[j]))
                {
                    TC_LOG_ERROR("sql", "Quest %u has `RewardFactionId%d` = %u but raw faction (faction.dbc) %u does not exist, quest will not reward reputation for this faction.", qinfo->GetQuestId(), j+1, qinfo->RewardFactionId[j], qinfo->RewardFactionId[j]);
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RewardFactionId%d` = 0 but `RewardFactionValueIdOverride%d` = %i.",
        ...
        if (qinfo->RewardSpell)
        {
            ...
            if (!spellInfo)
            {
                TC_LOG_ERROR("sql", "Quest %u has `RewardSpell` = %u but spell %u does not exist, spell removed as display reward.",
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RewardSpell` = %u but spell %u is broken, quest will not have a spell reward.",
                    qinfo->GetQuestId(), qinfo->RewardSpell, qinfo->RewardSpell);
                qinfo->RewardSpell = 0;                        // no spell reward will display for this quest
            }
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RewardSpell` = %u but spell %u is talent, quest will not have a spell reward.",
                    qinfo->GetQuestId(), qinfo->RewardSpell, qinfo->RewardSpell);
                qinfo->RewardSpell = 0;                        // no spell reward will display for this quest
            }*/
        }
        ...
        if (qinfo->RewardSpellCast > 0)
        {
            ...
            if (!spellInfo)
            {
                TC_LOG_ERROR("sql", "Quest %u has `RewardSpellCast` = %u but spell %u does not exist, quest will not have a spell reward.",
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RewardSpellCast` = %u but spell %u is broken, quest will not have a spell reward.",
                    qinfo->GetQuestId(), qinfo->RewardSpellCast, qinfo->RewardSpellCast);
                qinfo->RewardSpellCast = 0;                    // no spell will be casted on player
            }
        ...
        if (qinfo->RewardMailTemplateId)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RewardMailTemplateId` = %u but mail template  %u does not exist, quest will not have a mail reward.",
            ...
            else if (usedMailTemplates.find(qinfo->RewardMailTemplateId) != usedMailTemplates.end())
            {
                ...
                TC_LOG_ERROR("sql", "Quest %u has `RewardMailTemplateId` = %u but mail template  %u already used for quest %u, quest will not have a mail reward.",
        ...
        if (qinfo->NextQuestIdChain)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `NextQuestIdChain` = %u but quest %u does not exist, quest chain will not work.",
        ...
        for (uint8 j = 0; j < QUEST_REWARD_CURRENCY_COUNT; ++j)
        {
            if (qinfo->RewardCurrencyId[j])
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Quest %u has `RewardCurrencyId%d` = %u but `RewardCurrencyCount%d` = 0, quest can't be done.",
                ...
                {
                    TC_LOG_ERROR("sql", "Quest %u has `RewardCurrencyId%d` = %u but currency with entry %u does not exist, quest can't be done.",
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RewardCurrencyId%d` = 0 but `RewardCurrencyCount%d` = %u, quest can't be done.",
        ...
        for (uint8 j = 0; j < QUEST_REQUIRED_CURRENCY_COUNT; ++j)
        {
            if (qinfo->RequiredCurrencyId[j])
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Quest %u has `RequiredCurrencyId%d` = %u but `RequiredCurrencyCount%d` = 0, quest can't be done.",
                ...
                {
                    TC_LOG_ERROR("sql", "Quest %u has `RequiredCurrencyId%d` = %u but currency with entry %u does not exist, quest can't be done.",
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RequiredCurrencyId%d` = 0 but `RequiredCurrencyCount%d` = %u, quest can't be done.",
        ...
        if (qinfo->SoundAccept)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `SoundAccept` = %u but sound %u does not exist, set to 0.",
        ...
        if (qinfo->SoundTurnIn)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `SoundTurnIn` = %u but sound %u does not exist, set to 0.",
        ...
        if (qinfo->RequiredSpell > 0)
        {
            ...
            if (!spellInfo)
            {
                TC_LOG_ERROR("sql", "Quest %u has `RequiredSpell` = %u but spell %u does not exist, quest will not require a spell.",
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RequiredSpell` = %u but spell %u is broken, quest will not require a spell.",
                    qinfo->GetQuestId(), qinfo->RequiredSpell, qinfo->RequiredSpell);
                qinfo->RequiredSpell = 0;                    // no spell will be required
            }
        ...
        if (qinfo->RewardSkillId)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RewardSkillId` = %u but this skill does not exist",
                    qinfo->GetQuestId(), qinfo->RewardSkillId);
            }
            if (!qinfo->RewardSkillPoints)
            {
                TC_LOG_ERROR("sql", "Quest %u has `RewardSkillId` = %u but `RewardSkillPoints` is 0",
                    qinfo->GetQuestId(), qinfo->RewardSkillId);
            }
        }
        ...
        if (qinfo->RewardSkillPoints)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Quest %u has `RewardSkillPoints` = %u but max possible skill is %u, quest can't be done.",
                    qinfo->GetQuestId(), qinfo->RewardSkillPoints, sWorld->GetConfigMaxSkillValue());
                // no changes, quest can't be done for this requirement
            }
            if (!qinfo->RewardSkillId)
            {
                TC_LOG_ERROR("sql", "Quest %u has `RewardSkillPoints` = %u but `RewardSkillId` is 0",
        ...
        if (qinfo->PrevQuestId)
        {
            if (_questTemplates.find(abs(qinfo->GetPrevQuestId())) == _questTemplates.end())
                TC_LOG_ERROR("sql", "Quest %d has PrevQuestId %i, but no such quest", qinfo->GetQuestId(), qinfo->GetPrevQuestId());
            else
        ...
        if (qinfo->NextQuestId)
        {
            ...
            if (qNextItr == _questTemplates.end())
                TC_LOG_ERROR("sql", "Quest %d has NextQuestId %i, but no such quest", qinfo->GetQuestId(), qinfo->GetNextQuestId());
            else
            {
    ...
    for (uint32 i = 0; i < sSpellMgr->GetSpellInfoStoreSize(); ++i)
    {
        ...
        for (uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Spell (id: %u) have SPELL_EFFECT_QUEST_COMPLETE for quest %u, but quest not have flag QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT. Quest flags must be fixed, quest modified to enable objective.", spellInfo->Id, quest_id);
    ...
    TC_LOG_INFO("server", ">> Loaded %lu quests definitions in %u ms", (unsigned long)_questTemplates.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadQuestLocales()
{
    ...
    TC_LOG_INFO("server", ">> Loaded %lu Quest locale strings in %u ms", (unsigned long)_questLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadScripts(ScriptsType type)
{
    ...
    TC_LOG_INFO("server", "Loading %s...", tableName.c_str());
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 script definitions. DB table `%s` is empty!", tableName.c_str());
        return;
    }
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            case SCRIPT_COMMAND_TALK:
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` has invalid talk type (datalong = %u) in SCRIPT_COMMAND_TALK for script id %u",
                        tableName.c_str(), tmp.Talk.ChatType, tmp.id);
                    continue;
                }
                if (!tmp.Talk.TextID)
                {
                    TC_LOG_ERROR("sql", "Table `%s` has invalid talk text id (dataint = %i) in SCRIPT_COMMAND_TALK for script id %u",
                        tableName.c_str(), tmp.Talk.TextID, tmp.id);
                    continue;
                }
                if (tmp.Talk.TextID < MIN_DB_SCRIPT_STRING_ID || tmp.Talk.TextID >= MAX_DB_SCRIPT_STRING_ID)
                {
                    TC_LOG_ERROR("sql", "Table `%s` has out of range text id (dataint = %i expected %u-%u) in SCRIPT_COMMAND_TALK for script id %u",
            ...
            case SCRIPT_COMMAND_EMOTE:
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` has invalid emote id (datalong = %u) in SCRIPT_COMMAND_EMOTE for script id %u",
            ...
            case SCRIPT_COMMAND_TELEPORT_TO:
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` has invalid map (Id: %u) in SCRIPT_COMMAND_TELEPORT_TO for script id %u",
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` has invalid coordinates (X: %f Y: %f Z: %f O: %f) in SCRIPT_COMMAND_TELEPORT_TO for script id %u",
            ...
            case SCRIPT_COMMAND_QUEST_EXPLORED:
            {
                ...
                if (!quest)
                {
                    TC_LOG_ERROR("sql", "Table `%s` has invalid quest (ID: %u) in SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id %u",
                ...
                if (!quest->HasSpecialFlag(QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT))
                {
                    TC_LOG_ERROR("sql", "Table `%s` has quest (ID: %u) in SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id %u, but quest not have flag QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT in quest flags. Script command or quest flags wrong. Quest modified to require objective.",
                        tableName.c_str(), tmp.QuestExplored.QuestID, tmp.id);
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` has too large distance (%u) for exploring objective complete in `datalong2` in SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id %u",
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` has too large distance (%u) for exploring objective complete in `datalong2` in SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id %u, max distance is %f or 0 for disable distance check",
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` has too small distance (%u) for exploring objective complete in `datalong2` in SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id %u, min distance is %f or 0 for disable distance check",
            ...
            case SCRIPT_COMMAND_KILL_CREDIT:
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` has invalid creature (Entry: %u) in SCRIPT_COMMAND_KILL_CREDIT for script id %u",
            ...
            case SCRIPT_COMMAND_RESPAWN_GAMEOBJECT:
            {
                ...
                if (!data)
                {
                    TC_LOG_ERROR("sql", "Table `%s` has invalid gameobject (GUID: %u) in SCRIPT_COMMAND_RESPAWN_GAMEOBJECT for script id %u",
                ...
                if (!info)
                {
                    TC_LOG_ERROR("sql", "Table `%s` has gameobject with invalid entry (GUID: %u Entry: %u) in SCRIPT_COMMAND_RESPAWN_GAMEOBJECT for script id %u",
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` have gameobject type (%u) unsupported by command SCRIPT_COMMAND_RESPAWN_GAMEOBJECT for script id %u",
            ...
            case SCRIPT_COMMAND_TEMP_SUMMON_CREATURE:
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` has invalid coordinates (X: %f Y: %f Z: %f O: %f) in SCRIPT_COMMAND_TEMP_SUMMON_CREATURE for script id %u",
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` has invalid creature (Entry: %u) in SCRIPT_COMMAND_TEMP_SUMMON_CREATURE for script id %u",
            ...
            case SCRIPT_COMMAND_CLOSE_DOOR:
            {
                ...
                if (!data)
                {
                    TC_LOG_ERROR("sql", "Table `%s` has invalid gameobject (GUID: %u) in %s for script id %u",
                ...
                if (!info)
                {
                    TC_LOG_ERROR("sql", "Table `%s` has gameobject with invalid entry (GUID: %u Entry: %u) in %s for script id %u",
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` has gameobject type (%u) non supported by command %s for script id %u",
            ...
            case SCRIPT_COMMAND_REMOVE_AURA:
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` using non-existent spell (id: %u) in SCRIPT_COMMAND_REMOVE_AURA for script id %u",
                        tableName.c_str(), tmp.RemoveAura.SpellID, tmp.id);
                    continue;
                }
                if (tmp.RemoveAura.Flags & ~0x1)                    // 1 bits (0, 1)
                {
                    TC_LOG_ERROR("sql", "Table `%s` using unknown flags in datalong2 (%u) in SCRIPT_COMMAND_REMOVE_AURA for script id %u",
            ...
            case SCRIPT_COMMAND_CAST_SPELL:
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` using non-existent spell (id: %u) in SCRIPT_COMMAND_CAST_SPELL for script id %u",
                        tableName.c_str(), tmp.CastSpell.SpellID, tmp.id);
                    continue;
                }
                if (tmp.CastSpell.Flags > 4)                      // targeting type
                {
                    TC_LOG_ERROR("sql", "Table `%s` using unknown target in datalong2 (%u) in SCRIPT_COMMAND_CAST_SPELL for script id %u",
                        tableName.c_str(), tmp.CastSpell.Flags, tmp.id);
                    continue;
                }
                if (tmp.CastSpell.Flags != 4 && tmp.CastSpell.CreatureEntry & ~0x1)                      // 1 bit (0, 1)
                {
                    TC_LOG_ERROR("sql", "Table `%s` using unknown flags in dataint (%u) in SCRIPT_COMMAND_CAST_SPELL for script id %u",
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` using invalid creature entry in dataint (%u) in SCRIPT_COMMAND_CAST_SPELL for script id %u",
                        tableName.c_str(), tmp.CastSpell.CreatureEntry, tmp.id);
                    continue;
                }
                break;
            ...
            case SCRIPT_COMMAND_CREATE_ITEM:
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Table `%s` has nonexistent item (entry: %u) in SCRIPT_COMMAND_CREATE_ITEM for script id %u",
                        tableName.c_str(), tmp.CreateItem.ItemEntry, tmp.id);
                    continue;
                }
                if (!tmp.CreateItem.Amount)
                {
                    TC_LOG_ERROR("sql", "Table `%s` SCRIPT_COMMAND_CREATE_ITEM but amount is %u for script id %u",
    ...
    TC_LOG_INFO("server", ">> Loaded %u script definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadGameObjectScripts()
{
    ...
    for (ScriptMapMap::const_iterator itr = sGameObjectScripts.begin(); itr != sGameObjectScripts.end(); ++itr)
    {
        if (!GetGOData(itr->first))
            TC_LOG_ERROR("sql", "Table `gameobject_scripts` has not existing gameobject (GUID: %u) as script id", itr->first);
    }
}
...
void ObjectMgr::LoadQuestEndScripts()
{
    ...
    for (ScriptMapMap::const_iterator itr = sQuestEndScripts.begin(); itr != sQuestEndScripts.end(); ++itr)
    {
        if (!GetQuestTemplate(itr->first))
            TC_LOG_ERROR("sql", "Table `quest_end_scripts` has not existing quest (Id: %u) as script id", itr->first);
    }
}
...
void ObjectMgr::LoadQuestStartScripts()
{
    ...
    {
        if (!GetQuestTemplate(itr->first))
            TC_LOG_ERROR("sql", "Table `quest_start_scripts` has not existing quest (Id: %u) as script id", itr->first);
    }
}
...
void ObjectMgr::LoadSpellScripts()
{
    ...
    for (ScriptMapMap::const_iterator itr = sSpellScripts.begin(); itr != sSpellScripts.end(); ++itr)
    {
        ...
        if (!spellInfo)
        {
            TC_LOG_ERROR("sql", "Table `spell_scripts` has not existing spell (Id: %u) as script id", spellId);
            continue;
        }
        ...
        if (!spellInfo->Effects[i]->Effect || (spellInfo->Effects[i]->Effect != SPELL_EFFECT_SCRIPT_EFFECT && spellInfo->Effects[i]->Effect != SPELL_EFFECT_DUMMY))
            TC_LOG_ERROR("sql", "Table `spell_scripts` - spell %u effect %u is not SPELL_EFFECT_SCRIPT_EFFECT or SPELL_EFFECT_DUMMY", spellId, i);
    }
}
...
void ObjectMgr::LoadEventScripts()
{
    ...
    for (ScriptMapMap::const_iterator itr = sEventScripts.begin(); itr != sEventScripts.end(); ++itr)
    {
        ...
        if (itr2 == evt_scripts.end())
            TC_LOG_ERROR("sql", "Table `event_scripts` has script (Id: %u) not referring to any gameobject_template type 10 data2 field, type 3 data6 field, type 13 data 2 field or any spell effect %u",
...
void ObjectMgr::LoadWaypointScripts()
{
    ...
    for (std::set<uint32>::iterator itr = actionSet.begin(); itr != actionSet.end(); ++itr)
        TC_LOG_ERROR("sql", "There is no waypoint which links to the waypoint script %u", *itr);
}
...
void ObjectMgr::LoadSpellScriptNames()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 spell script names. DB table `spell_script_names` is empty!");
        return;
    }
    ...
    do
    {
        ...
        if (!spellInfo)
        {
            TC_LOG_ERROR("sql", "Scriptname:`%s` spell (spell_id:%d) does not exist in `Spell.dbc`.", scriptName, fields[0].GetInt32());
            continue;
        }
        ...
        if (allRanks)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Scriptname:`%s` spell (spell_id:%d) is not first rank of spell.", scriptName, fields[0].GetInt32());
                continue;
            }
            while (spellInfo)
            {
    ...
    TC_LOG_INFO("server", ">> Loaded %u spell script names in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadAchievementScriptNames()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 achievement script names. DB table `achievement_script_names` is empty!");
        return;
    }
    ...
    do
    {
        ...
        if (!achievement)
        {
            TC_LOG_ERROR("sql", "Scriptname:`%s` achievement (id:%d) does not exist in `Achievement.dbc`.", scriptName, fields[0].GetInt32());
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u achievement script names in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::ValidateSpellScripts()
{
    ...
    {
        TC_LOG_INFO("server", ">> Validated 0 scripts.");
        return;
    }
    ...
    for (SpellScriptsContainer::iterator itr = _spellScriptsStore.begin(); itr != _spellScriptsStore.end();)
    {
        ...
        for (std::vector<std::pair<SpellScriptLoader *, SpellScriptsContainer::iterator> >::iterator sitr = SpellScriptLoaders.begin(); sitr != SpellScriptLoaders.end(); ++sitr)
        {
            ...
            {
                TC_LOG_ERROR("server", "Functions GetSpellScript() and GetAuraScript() of script `%s` do not return objects - script skipped",  GetScriptName(sitr->second->second));
                valid = false;
            }
            if (spellScript)
            {
    ...
    TC_LOG_INFO("server", ">> Validated %u scripts in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPageTexts()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 page texts. DB table `page_text` is empty!");
        return;
    }
    ...
    for (PageTextContainer::const_iterator itr = _pageTextStore.begin(); itr != _pageTextStore.end(); ++itr)
    {
        if (itr->second.NextPage)
        {
            ...
            if (itr2 == _pageTextStore.end())
                TC_LOG_ERROR("sql", "Page text (Id: %u) has not existing next page (Id: %u)", itr->first, itr->second.NextPage);

        }
    }

    TC_LOG_INFO("server", ">> Loaded %u page texts in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPageTextLocales()
{
    ...
    TC_LOG_INFO("server", ">> Loaded %lu PageText locale strings in %u ms", (unsigned long)_pageTextLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadInstanceTemplate()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 instance templates. DB table `page_text` is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql", "ObjectMgr::LoadInstanceTemplate: bad mapid %d for template!", mapID);
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u instance templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadInstanceEncounters()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 instance encounters, table is empty!");
    ...
    do
    {
        ...
        if (!dungeonEncounter)
        {
            TC_LOG_ERROR("sql", "Table `instance_encounters` has an invalid encounter id %u, skipped!", entry);
            continue;
        }

        if (lastEncounterDungeon && !sLFGMgr->GetLFGDungeonEntry(lastEncounterDungeon))
        {
            TC_LOG_ERROR("sql", "Table `instance_encounters` has an encounter %u (%s) marked as final for invalid dungeon id %u, skipped!", entry, dungeonEncounter->encounterName, lastEncounterDungeon);
            continue;
        }
        ...
        if (lastEncounterDungeon)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Table `instance_encounters` specified encounter %u (%s) as last encounter but %u (%s) is already marked as one, skipped!", entry, dungeonEncounter->encounterName, itr->second->id, itr->second->encounterName);
                continue;
            }
        ...
        switch (creditType)
        {
            case ENCOUNTER_CREDIT_KILL_CREATURE:
            {
                ...
                if (!creatureInfo)
                {
                    TC_LOG_ERROR("sql", "Table `instance_encounters` has an invalid creature (entry %u) linked to the encounter %u (%s), skipped!", creditEntry, entry, dungeonEncounter->encounterName);
                    continue;
                }
            ...
            case ENCOUNTER_CREDIT_CAST_SPELL:
                ...
                {
                    TC_LOG_ERROR("sql", "Table `instance_encounters` has an invalid spell (entry %u) linked to the encounter %u (%s), skipped!", creditEntry, entry, dungeonEncounter->encounterName);
                    continue;
                }
                break;
            default:
                TC_LOG_ERROR("sql", "Table `instance_encounters` has an invalid credit type (%u) for encounter %u (%s), skipped!", creditType, entry, dungeonEncounter->encounterName);
                continue;
        }

        if (dungeonEncounter->difficulty > 0 && dungeonEncounter->difficulty != DungeonDifficult)
        {
            TC_LOG_ERROR("sql", "Table `instance_encounters` overvrite dificult from dbc for encounter %u. DBC %i, DB %i", entry, dungeonEncounter->difficulty, DungeonDifficult);
    ...
    TC_LOG_INFO("server", ">> Loaded %u instance encounters in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadGossipText()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded %u npc texts", count);
        return;
    }
    ...
    do
    {
        ...
        if (!Text_ID)
        {
            TC_LOG_ERROR("sql", "Table `npc_text` has record wit reserved id 0, ignore.");
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u npc texts in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadNpcTextLocales()
{
    ...
    TC_LOG_INFO("server", ">> Loaded %lu NpcText locale strings in %u ms", (unsigned long)_npcTextLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::ReturnOrDeleteOldMails(bool serverUp)
{
    ...
    TC_LOG_INFO("server", "Returning mails current time: hour: %d, minute: %d, second: %d ", lt->tm_hour, lt->tm_min, lt->tm_sec);
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> No expired mails found.");
    ...
    TC_LOG_INFO("server", ">> Processed %u expired mails: %u deleted and %u returned in %u ms", deletedCount + returnedCount, deletedCount, returnedCount, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadQuestAreaTriggers()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 quest trigger points. DB table `areatrigger_involvedrelation` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!atEntry)
        {
            TC_LOG_ERROR("sql", "Area trigger (ID:%u) does not exist in `AreaTrigger.dbc`.", trigger_ID);
            continue;
        }
        ...
        if (!quest)
        {
            TC_LOG_ERROR("sql", "Table `areatrigger_involvedrelation` has record (id: %u) for not existing quest %u", trigger_ID, quest_ID);
            continue;
        }

        if (!quest->HasSpecialFlag(QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT))
        {
            TC_LOG_ERROR("sql", "Table `areatrigger_involvedrelation` has record (id: %u) for not quest %u, but quest not have flag QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT. Trigger or quest flags must be fixed, quest modified to require objective.", trigger_ID, quest_ID);
    ...
    TC_LOG_INFO("server", ">> Loaded %u quest trigger points in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadTavernAreaTriggers()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 tavern triggers. DB table `areatrigger_tavern` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server", ">> Loaded %u tavern triggers in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadAreaTriggerScripts()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 areatrigger scripts. DB table `areatrigger_scripts` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server", ">> Loaded %u areatrigger scripts in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
uint32 ObjectMgr::GetTaxiMountDisplayId(uint32 id, uint32 team, bool allowed_alt_team /* = false */)
{
    ...
    if (node)
    {
        ...
        if (mount_info)
        {
            ...
            if (!mount_id)
            {
                TC_LOG_ERROR("sql", "No displayid found for the taxi mount with the entry %u! Can't load it!", mount_entry);
                return false;
            }
        }
    }
...
void ObjectMgr::LoadGraveyardZones()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 graveyard-zone links. DB table `game_graveyard_zone` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!entry)
        {
            TC_LOG_ERROR("sql", "Table `game_graveyard_zone` has a record for not existing graveyard (WorldSafeLocs.dbc id) %u, skipped.", safeLocId);
            continue;
        }
        ...
        /*if (areaEntry->zone != 0)
        {
            TC_LOG_ERROR("sql", "Table `game_graveyard_zone` has a record for subzone id (%u) instead of zone, skipped.", zoneId);
            continue;
        }*/

        if (team != 0 && team != HORDE && team != ALLIANCE)
        {
            TC_LOG_ERROR("sql", "Table `game_graveyard_zone` has a record for non player faction (%u), skipped.", team);
            continue;
        }

        if (!AddGraveYardLink(safeLocId, zoneId, team, false))
            TC_LOG_ERROR("sql", "Table `game_graveyard_zone` has a duplicate record for Graveyard (ID: %u) and Zone (ID: %u), skipped.", safeLocId, zoneId);
    } while (result->NextRow());

    TC_LOG_INFO("server", ">> Loaded %u graveyard-zone links in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
WorldSafeLocsEntry const* ObjectMgr::GetClosestGraveYard(float x, float y, float z, uint32 MapId, uint32 team)
{
    ...
    if (!zoneId)
    {
        if (z > -500)
        {
            TC_LOG_ERROR("server", "ZoneId not found for map %u coords (%f, %f, %f)", MapId, x, y, z);
    ...
    {
        TC_LOG_ERROR("sql", "Table `game_graveyard_zone` incomplete: Zone %u Team %u does not have a linked graveyard.", zoneId, team);
        return NULL;
    }
    ...
    for (GraveYardContainer::const_iterator itr = graveLow; itr != graveUp; ++itr)
    {
        ...
        if (!entry)
        {
            TC_LOG_ERROR("sql", "Table `game_graveyard_zone` has record for not existing graveyard (WorldSafeLocs.dbc id) %u, skipped.", data.safeLocId);
            continue;
        }
...
void ObjectMgr::LoadAreaTriggerTeleports()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 area trigger teleport definitions. DB table `areatrigger_teleport` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!mapEntry)
        {
            TC_LOG_ERROR("sql", "Area trigger (ID:%u) target map (ID: %u) does not exist in `Map.dbc`.", Trigger_ID, at.target_mapId);
            continue;
        }

        if (at.target_X == 0 && at.target_Y == 0 && at.target_Z == 0)
        {
            TC_LOG_ERROR("sql", "Area trigger (ID:%u) target coordinates not provided.", Trigger_ID);
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u area trigger teleport definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadAccessRequirements()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 access requirement definitions. DB table `access_requirement` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (ar.item)
        {
            ...
            if (!pProto)
            {
                TC_LOG_ERROR("server", "Key item %u does not exist for map %u difficulty %u, removing key requirement.", ar.item, mapid, difficulty);
                ar.item = 0;
            }
        }
        ...
        if (ar.item2)
        {
            ...
            if (!pProto)
            {
                TC_LOG_ERROR("server", "Second item %u does not exist for map %u difficulty %u, removing key requirement.", ar.item2, mapid, difficulty);
                ar.item2 = 0;
            }
        }
        ...
        if (ar.quest_A)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Required Alliance Quest %u not exist for map %u difficulty %u, remove quest done requirement.", ar.quest_A, mapid, difficulty);
                ar.quest_A = 0;
            }
        }
        ...
        if (ar.quest_H)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Required Horde Quest %u not exist for map %u difficulty %u, remove quest done requirement.", ar.quest_H, mapid, difficulty);
                ar.quest_H = 0;
            }
        }
        ...
        if (ar.achievement)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Required Achievement %u not exist for map %u difficulty %u, remove quest done requirement.", ar.achievement, mapid, difficulty);
                ar.achievement = 0;
            }
        }
        ...
        if (ar.achievement_A)
        {
            ...
            {
                TC_LOG_ERROR("sql", "Required achievement_A %u not exist for map %u difficulty %u, remove quest done requirement.", ar.achievement_A, mapid, difficulty);
                ar.achievement_A = 0;
            }
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u access requirement definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
uint64 ObjectMgr::GenerateEquipmentSetGuid()
{
    ...
    {
        TC_LOG_ERROR("server", "EquipmentSet guid overflow!! Can't continue, shutting down server. ");
        World::StopNow(ERROR_EXIT_CODE);
    }
...
GuidDiapason ObjectMgr::GenerateNewDiapasonFor(uint32 type, uint32 count)
{
    ...
    switch (type)
    {
        ...
        default:
            TC_LOG_ERROR("sql", "Type not support.");
            break;
    }
...
void ObjectMgr::LoadGameObjectLocales()
{
    ...
    TC_LOG_INFO("server", ">> Loaded %lu gameobject locale strings in %u ms", (unsigned long)_gameObjectLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
inline void CheckGOLockId(GameObjectTemplate const* goInfo, uint32 dataN, uint32 N)
{
    ...
    TC_LOG_ERROR("sql", "Gameobject (Entry: %u GoType: %u) have data%d=%u but lock (Id: %u) not found.",
...
inline void CheckGOLinkedTrapId(GameObjectTemplate const* goInfo, uint32 dataN, uint32 N)
{
    if (GameObjectTemplate const* trapInfo = sObjectMgr->GetGameObjectTemplate(dataN))
    {
        if (trapInfo->type != GAMEOBJECT_TYPE_TRAP)
            TC_LOG_ERROR("sql", "Gameobject (Entry: %u GoType: %u) have data%d=%u but GO (Entry %u) have not GAMEOBJECT_TYPE_TRAP (%u) type.",
...
inline void CheckGOSpellId(GameObjectTemplate const* goInfo, uint32 dataN, uint32 N)
{
    ...
    TC_LOG_ERROR("sql", "Gameobject (Entry: %u GoType: %u) have data%d=%u but Spell (Entry %u) not exist.",
...
inline void CheckAndFixGOChairHeightId(GameObjectTemplate const* goInfo, uint32 const& dataN, uint32 N)
{
    ...
    TC_LOG_ERROR("sql", "Gameobject (Entry: %u GoType: %u) have data%d=%u but correct chair height in range 0..%i.",
...
inline void CheckGONoDamageImmuneId(GameObjectTemplate* goTemplate, uint32 dataN, uint32 N)
{
    ...
    TC_LOG_ERROR("sql", "Gameobject (Entry: %u GoType: %u) have data%d=%u but expected boolean (0/1) noDamageImmune field value.", goTemplate->entry, goTemplate->type, N, dataN);
}
...
inline void CheckGOConsumable(GameObjectTemplate const* goInfo, uint32 dataN, uint32 N)
{
    ...
    TC_LOG_ERROR("sql", "Gameobject (Entry: %u GoType: %u) have data%d=%u but expected boolean (0/1) consumable field value.",
...
void ObjectMgr::LoadGameObjectTemplate()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 gameobject definitions. DB table `gameobject_template` is empty.");
        return;
    }
    ...
    do
    {
        ...
        switch (got.type)
        {
            ...
            case GAMEOBJECT_TYPE_SPELL_FOCUS:               //8
            {
                if (got.spellFocus.spellFocusType)
                {
                    if (!sSpellFocusObjectStore.LookupEntry(got.spellFocus.spellFocusType))
                        TC_LOG_ERROR("sql", "GameObject (Entry: %u GoType: %u) have data0=%u but SpellFocus (Id: %u) not exist.",
            ...
            case GAMEOBJECT_TYPE_GOOBER:                    //10
            {
                ...
                if (got.goober.pageID)                  // pageID
                {
                    if (!GetPageText(got.goober.pageID))
                        TC_LOG_ERROR("sql", "GameObject (Entry: %u GoType: %u) have data7=%u but PageText (Entry %u) not exist.",
            ...
            case GAMEOBJECT_TYPE_TRANSPORT:                 // 11
            {
                ...
                {
                    TC_LOG_ERROR("sql", "Gameobject (Entry: %u GoType: %u) is transport by does not have entries in TransportAnimation.dbc! Gameobject is obsolete.",
                        got.entry, got.type);
                    break;
                }
                ...
                if (uint32 frame = got.transport.Timeto2ndfloor)
                {
                    ...
                    {
                        TC_LOG_ERROR("sql", "Gameobject (Entry: %u GoType: %u) has data0=%u but this frame is not in TransportAnimation.dbc!",
                ...
                if (uint32 frame = got.transport.Timeto3rdfloor)
                {
                    if (itr->second.find(frame) == itr->second.end())
                    {
                        TC_LOG_ERROR("sql", "Gameobject (Entry: %u GoType: %u) has data6=%u but this frame is not in TransportAnimation.dbc!",
                            got.entry, got.type, frame);
                    }
                }
                ...
                if (uint32 frame = got.transport.Timeto4thfloor)
                {
                    if (itr->second.find(frame) == itr->second.end())
                    {
                        TC_LOG_ERROR("sql", "Gameobject (Entry: %u GoType: %u) has data8=%u but this frame is not in TransportAnimation.dbc!",
                            got.entry, got.type, frame);
                    }
                }
                ...
                if (uint32 frame = got.transport.Timeto5thfloor)
                {
                    if (itr->second.find(frame) == itr->second.end())
                    {
                        TC_LOG_ERROR("sql", "Gameobject (Entry: %u GoType: %u) has data10=%u but this frame is not in TransportAnimation.dbc!",
                            got.entry, got.type, frame);
                    }
                }
                break;
            ...
            case GAMEOBJECT_TYPE_MAP_OBJ_TRANSPORT:              //15
            {
                if (got.moTransport.taxiPathID)
                {
                    if (got.moTransport.taxiPathID >= sTaxiPathNodesByPath.size() || sTaxiPathNodesByPath[got.moTransport.taxiPathID].empty())
                        TC_LOG_ERROR("sql", "GameObject (Entry: %u GoType: %u) have data0=%u but TaxiPath (Id: %u) not exist.",
    ...
    TC_LOG_INFO("server", ">> Loaded %u game object templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadExplorationBaseXP()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 BaseXP definitions. DB table `exploration_basexp` is empty.");
    ...
    TC_LOG_INFO("server", ">> Loaded %u BaseXP definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPetNames()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 pet name parts. DB table `pet_name_generation` is empty!");
        return;
    }
    ...
    TC_LOG_INFO("server", ">> Loaded %u pet name parts in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPetNumber()
{
    ...
    TC_LOG_INFO("server", ">> Loaded the max pet number: %d in %u ms", _hiPetNumber-1, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadBattlePetGuid()
{
    ...
    TC_LOG_INFO("server", ">> Loaded the max battle pet guid: %d in %u ms", _hiBattlePetGuid-1, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCurrencysLoot()
{
    ...
    do
    {
        ...
        if (type < 1)
        {
            TC_LOG_INFO("server", "Currency 'type' can not be < 1 (entry = %u type = %i)", entry, type);
            continue;
        }
        else if (type > 3)
        {
            TC_LOG_INFO("server", "Currency 'type' can not be > 3 (entry = %u type = %i)", entry, type);
            continue;
        }
    ...
    if (count)
        TC_LOG_INFO("server", "Loaded %u currency loot definition", count);
    else
        TC_LOG_INFO("server", "Loaded 0 currency loot definition. Table is empty!");
}
...
void ObjectMgr::LoadCorpses()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 corpses. DB table `corpse` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("server", "Corpse (guid: %u) have wrong corpse type (%u), not loading.", guid, type);
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u corpses in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadReputationRewardRate()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("sql", ">> Loaded `reputation_reward_rate`, table is empty!");
    ...
    do
    {
        ...
        if (!factionEntry)
        {
            TC_LOG_ERROR("sql", "Faction (faction.dbc) %u does not exist but is used in `reputation_reward_rate`", factionId);
            continue;
        }

        if (repRate.quest_rate < 0.0f)
        {
            TC_LOG_ERROR("sql", "Table reputation_reward_rate has quest_rate with invalid rate %f, skipping data for faction %u", repRate.quest_rate, factionId);
            continue;
        }

        if (repRate.creature_rate < 0.0f)
        {
            TC_LOG_ERROR("sql", "Table reputation_reward_rate has creature_rate with invalid rate %f, skipping data for faction %u", repRate.creature_rate, factionId);
            continue;
        }

        if (repRate.spell_rate < 0.0f)
        {
            TC_LOG_ERROR("sql", "Table reputation_reward_rate has spell_rate with invalid rate %f, skipping data for faction %u", repRate.spell_rate, factionId);
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u reputation_reward_rate in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadReputationOnKill()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 creature award reputation definitions. DB table `creature_onkill_reputation` is empty.");
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql", "Table `creature_onkill_reputation` have data for not existed creature entry (%u), skipped", creature_id);
            continue;
        }
        ...
        if (repOnKill.RepFaction1)
        {
            ...
            if (!factionEntry1)
            {
                TC_LOG_ERROR("sql", "Faction (faction.dbc) %u does not exist but is used in `creature_onkill_reputation`", repOnKill.RepFaction1);
                continue;
            }
        }
        ...
        if (repOnKill.RepFaction2)
        {
            ...
            if (!factionEntry2)
            {
                TC_LOG_ERROR("sql", "Faction (faction.dbc) %u does not exist but is used in `creature_onkill_reputation`", repOnKill.RepFaction2);
                continue;
            }
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u creature award reputation definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadReputationSpilloverTemplate()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded `reputation_spillover_template`, table is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!factionEntry)
        {
            TC_LOG_ERROR("sql", "Faction (faction.dbc) %u does not exist but is used in `reputation_spillover_template`", factionId);
            continue;
        }

        if (factionEntry->team == 0)
        {
            TC_LOG_ERROR("sql", "Faction (faction.dbc) %u in `reputation_spillover_template` does not belong to any team, skipping", factionId);
            continue;
        }
        ...
        for (uint32 i = 0; i < MAX_SPILLOVER_FACTIONS; ++i)
        {
            if (repTemplate.faction[i])
            {
                ...
                if (!factionSpillover)
                {
                    TC_LOG_ERROR("sql", "Spillover faction (faction.dbc) %u does not exist but is used in `reputation_spillover_template` for faction %u, skipping", repTemplate.faction[i], factionId);
                    continue;
                }

                if (factionSpillover->reputationListID < 0)
                {
                    TC_LOG_ERROR("sql", "Spillover faction (faction.dbc) %u for faction %u in `reputation_spillover_template` can not be listed for client, and then useless, skipping", repTemplate.faction[i], factionId);
                    continue;
                }

                if (repTemplate.faction_rank[i] >= MAX_REPUTATION_RANK)
                {
                    TC_LOG_ERROR("sql", "Rank %u used in `reputation_spillover_template` for spillover faction %u is not valid, skipping", repTemplate.faction_rank[i], repTemplate.faction[i]);
                    continue;
                }
            }
        }
        ...
        {
            TC_LOG_ERROR("sql", "Faction (faction.dbc) %u does not exist but is used in `reputation_spillover_template`", repTemplate.faction[0]);
            continue;
        }
        FactionEntry const* factionEntry1 = sFactionStore.LookupEntry(repTemplate.faction[1]);
        if (repTemplate.faction[1] && !factionEntry1)
        {
            TC_LOG_ERROR("sql", "Faction (faction.dbc) %u does not exist but is used in `reputation_spillover_template`", repTemplate.faction[1]);
            continue;
        }
        FactionEntry const* factionEntry2 = sFactionStore.LookupEntry(repTemplate.faction[2]);
        if (repTemplate.faction[2] && !factionEntry2)
        {
            TC_LOG_ERROR("sql", "Faction (faction.dbc) %u does not exist but is used in `reputation_spillover_template`", repTemplate.faction[2]);
            continue;
        }
        FactionEntry const* factionEntry3 = sFactionStore.LookupEntry(repTemplate.faction[3]);
        if (repTemplate.faction[3] && !factionEntry3)
        {
            TC_LOG_ERROR("sql", "Faction (faction.dbc) %u does not exist but is used in `reputation_spillover_template`", repTemplate.faction[3]);
            continue;
        }
        FactionEntry const* factionEntry4 = sFactionStore.LookupEntry(repTemplate.faction[4]);
        if (repTemplate.faction[4] && !factionEntry4)
        {
            TC_LOG_ERROR("sql", "Faction (faction.dbc) %u does not exist but is used in `reputation_spillover_template`", repTemplate.faction[4]);
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u reputation_spillover_template in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPointsOfInterest()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 Points of Interest definitions. DB table `points_of_interest` is empty.");
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql", "Table `points_of_interest` (Entry: %u) have invalid coordinates (X: %f Y: %f), ignored.", point_id, POI.x, POI.y);
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u Points of Interest definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadQuestPOI()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 quest POI definitions. DB table `quest_poi` is empty.");
    ...
    TC_LOG_INFO("server", ">> Loaded %u quest POI definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadScenarioPOI()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 scenario POI definitions. DB table `scenario_poi` is empty.");
    ...
    TC_LOG_INFO("server", ">> Loaded %u scenario POI definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadNPCSpellClickSpells()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 spellclick spells. DB table `npc_spellclick_spells` is empty.");
    ...
    do
    {
        ...
        if (!cInfo)
        {
            TC_LOG_ERROR("sql", "Table npc_spellclick_spells references unknown creature_template %u. Skipping entry.", npc_entry);
            continue;
        }
        ...
        if (!spellinfo)
        {
            TC_LOG_ERROR("sql", "Table npc_spellclick_spells references unknown spellid %u. Skipping entry.", spellid);
            continue;
        }

        uint8 userType = fields[3].GetUInt16();
        if (userType >= SPELL_CLICK_USER_MAX)
            TC_LOG_ERROR("sql", "Table npc_spellclick_spells references unknown user type %u. Skipping entry.", uint32(userType));
    ...
    TC_LOG_INFO("server", ">> Loaded %u spellclick definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadQuestRelationsHelper(QuestRelations& map, QuestStarter& _map, std::string const& table, bool starter, bool go)
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 quest relations from `%s`, table is empty.", table.c_str());
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql", "Table `%s`: Quest %u listed for entry %u does not exist.", table.c_str(), quest, id);
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u quest relations from %s in %u ms", count, table.c_str(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadGameobjectQuestRelations()
{
    ...
    for (QuestRelations::iterator itr = _goQuestRelations.begin(); itr != _goQuestRelations.end(); ++itr)
    {
        ...
        if (!goInfo)
            TC_LOG_ERROR("sql", "Table `gameobject_questrelation` have data for not existed gameobject entry (%u) and existed quest %u", itr->first, itr->second);
        else if (goInfo->type != GAMEOBJECT_TYPE_QUESTGIVER)
            TC_LOG_ERROR("sql", "Table `gameobject_questrelation` have data gameobject entry (%u) for quest %u, but GO is not GAMEOBJECT_TYPE_QUESTGIVER", itr->first, itr->second);
    }
}
...
void ObjectMgr::LoadGameobjectInvolvedRelations()
{
    ...
    for (QuestRelations::iterator itr = _goQuestInvolvedRelations.begin(); itr != _goQuestInvolvedRelations.end(); ++itr)
    {
        GameObjectTemplate const* goInfo = GetGameObjectTemplate(itr->first);
        if (!goInfo)
            TC_LOG_ERROR("sql", "Table `gameobject_involvedrelation` have data for not existed gameobject entry (%u) and existed quest %u", itr->first, itr->second);
        else if (goInfo->type != GAMEOBJECT_TYPE_QUESTGIVER)
            TC_LOG_ERROR("sql", "Table `gameobject_involvedrelation` have data gameobject entry (%u) for quest %u, but GO is not GAMEOBJECT_TYPE_QUESTGIVER", itr->first, itr->second);
    }
}
...
void ObjectMgr::LoadCreatureQuestRelations()
{
    ...
    for (QuestRelations::iterator itr = _creatureQuestRelations.begin(); itr != _creatureQuestRelations.end(); ++itr)
    {
        ...
        if (!cInfo)
            TC_LOG_ERROR("sql", "Table `creature_questrelation` have data for not existed creature entry (%u) and existed quest %u", itr->first, itr->second);
        else if (!(cInfo->npcflag & UNIT_NPC_FLAG_QUESTGIVER))
            TC_LOG_ERROR("sql", "Table `creature_questrelation` has creature entry (%u) for quest %u, but npcflag does not include UNIT_NPC_FLAG_QUESTGIVER", itr->first, itr->second);
    }
}
...
void ObjectMgr::LoadCreatureInvolvedRelations()
{
    ...
    for (QuestRelations::iterator itr = _creatureQuestInvolvedRelations.begin(); itr != _creatureQuestInvolvedRelations.end(); ++itr)
    {
        CreatureTemplate const* cInfo = GetCreatureTemplate(itr->first);
        if (!cInfo)
            TC_LOG_ERROR("sql", "Table `creature_involvedrelation` have data for not existed creature entry (%u) and existed quest %u", itr->first, itr->second);
        else if (!(cInfo->npcflag & UNIT_NPC_FLAG_QUESTGIVER))
            TC_LOG_ERROR("sql", "Table `creature_involvedrelation` has creature entry (%u) for quest %u, but npcflag does not include UNIT_NPC_FLAG_QUESTGIVER", itr->first, itr->second);
    }
}
...
void ObjectMgr::LoadReservedPlayersNames()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 reserved player names. DB table `reserved_name` is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("server", "Table `reserved_name` have invalid name: %s", name.c_str());
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u reserved player names in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadGameObjectForQuests()
{
    ...
    {
        TC_LOG_INFO("server", ">> Loaded 0 GameObjects for quests");
        return;
    }
    ...
    TC_LOG_INFO("server", ">> Loaded %u GameObjects for quests in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
bool ObjectMgr::LoadTrinityStrings(const char* table, int32 min_value, int32 max_value)
{
    ...
    if (start_value < 0)
    {
        ...
        {
            TC_LOG_ERROR("sql", "Table '%s' attempt loaded with invalid range (%d - %d), strings not loaded.", table, min_value, max_value);
            return false;
        }
    ...
    if (!result)
    {
        if (min_value == MIN_TRINITY_STRING_ID)              // error only in case internal strings
            TC_LOG_ERROR("server", ">>  Loaded 0 trinity strings. DB table `%s` is empty. Cannot continue.", table);
        else
            TC_LOG_INFO("server", ">> Loaded 0 string templates. DB table `%s` is empty.", table);
    ...
    do
    {
        ...
        if (entry == 0)
        {
            TC_LOG_ERROR("sql", "Table `%s` contain reserved entry 0, ignored.", table);
            continue;
        }
        else if (entry < start_value || entry >= end_value)
        {
            TC_LOG_ERROR("sql", "Table `%s` contain entry %i out of allowed range (%d - %d), ignored.", table, entry, min_value, max_value);
            continue;
        }
        ...
        {
            TC_LOG_ERROR("sql", "Table `%s` contain data for already loaded entry  %i (from another table?), ignored.", table, entry);
            continue;
        }
    ...
    if (min_value == MIN_TRINITY_STRING_ID)
        TC_LOG_INFO("server", ">> Loaded %u Trinity strings from table %s in %u ms", count, table, GetMSTimeDiffToNow(oldMSTime));
    else
        TC_LOG_INFO("server", ">> Loaded %u string templates from %s in %u ms", count, table, GetMSTimeDiffToNow(oldMSTime));
...
const char *ObjectMgr::GetTrinityString(int32 entry, LocaleConstant locale_idx) const
{
    ...
    if (entry > 0)
        TC_LOG_ERROR("sql", "Entry %i not found in `trinity_string` table.", entry);
    else
        TC_LOG_ERROR("sql", "Trinity string entry %i not found in DB.", entry);
    return "<error>";
}
...
void ObjectMgr::LoadFishingBaseSkillLevel()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 areas for fishing base skill level. DB table `skill_fishing_base_level` is empty.");
    ...
    TC_LOG_INFO("server", ">> Loaded %u areas for fishing base skill level in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadGameTele()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">>  Loaded 0 GameTeleports. DB table `game_tele` is empty!");
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql", "Wrong position for id %u (name: %s) in `game_tele` table, ignoring.", id, gt.name.c_str());
            continue;
        }

        if (!Utf8toWStr(gt.name, gt.wnameLow))
        {
            TC_LOG_ERROR("sql", "Wrong UTF8 name for id %u in `game_tele` table, ignoring.", id);
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u GameTeleports in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadMailLevelRewards()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">>  Loaded 0 level dependent mail rewards. DB table `mail_level_reward` is empty.");
    ...
    do
    {
        ...
        if (level > MAX_LEVEL)
        {
            TC_LOG_ERROR("sql", "Table `mail_level_reward` have data for level %u that more supported by client (%u), ignoring.", level, MAX_LEVEL);
            continue;
        }

        if (!(raceMask & RACEMASK_ALL_PLAYABLE))
        {
            TC_LOG_ERROR("sql", "Table `mail_level_reward` have raceMask (%u) for level %u that not include any player races, ignoring.", raceMask, level);
            continue;
        }

        if (!sMailTemplateStore.LookupEntry(mailTemplateId))
        {
            TC_LOG_ERROR("sql", "Table `mail_level_reward` have invalid mailTemplateId (%u) for level %u that invalid not include any player races, ignoring.", mailTemplateId, level);
            continue;
        }

        if (!GetCreatureTemplate(senderEntry))
        {
            TC_LOG_ERROR("sql", "Table `mail_level_reward` have not existed sender creature entry (%u) for level %u that invalid not include any player races, ignoring.", senderEntry, level);
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u level dependent mail rewards in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::AddSpellToTrainer(uint32 entry, uint32 spell, uint32 spellCost, uint32 reqSkill, uint32 reqSkillValue, uint32 reqLevel)
{
    ...
    if (!cInfo)
    {
        TC_LOG_ERROR("sql", "Table `npc_trainer` contains an entry for a non-existing creature template (Entry: %u), ignoring", entry);
        return;
    }

    if (!(cInfo->npcflag & UNIT_NPC_FLAG_TRAINER))
    {
        TC_LOG_ERROR("sql", "Table `npc_trainer` contains an entry for a creature template (Entry: %u) without trainer flag, ignoring", entry);
        return;
    }
    ...
    if (!spellinfo)
    {
        TC_LOG_ERROR("sql", "Table `npc_trainer` contains an entry (Entry: %u) for a non-existing spell (Spell: %u), ignoring", entry, spell);
        return;
    }
    ...
    {
        TC_LOG_ERROR("sql", "Table `npc_trainer` contains an entry (Entry: %u) for a broken spell (Spell: %u), ignoring", entry, spell);
        return;
    }

/*    if (GetTalentSpellCost(spell))
    {
        TC_LOG_ERROR("sql", "Table `npc_trainer` contains an entry (Entry: %u) for a non-existing spell (Spell: %u) which is a talent, ignoring", entry, spell);
        return;
    }*/
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        {
            TC_LOG_ERROR("sql", "Table `npc_trainer` has spell %u for trainer entry %u with learn effect which has incorrect target type, ignoring learn effect!", spell, entry);
            continue;
        }
...
void ObjectMgr::LoadTrainerSpell()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">>  Loaded 0 Trainers. DB table `npc_trainer` is empty!");
    ...
    TC_LOG_INFO("server", ">> Loaded %d Trainers in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadVendors()
{
    ...
    if (!result)
    {
        ...
        TC_LOG_ERROR("server", ">>  Loaded 0 Vendors. DB table `npc_vendor` is empty!");
        return;
    }
    ...
    TC_LOG_INFO("server", ">> Loaded %d Vendors in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadGossipMenu()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0  gossip_menu entries. DB table `gossip_menu` is empty!");
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql", "Table gossip_menu entry %u are using non-existing text_id %u", gMenu.entry, gMenu.text_id);
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u gossip_menu entries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadGossipMenuItems()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 gossip_menu_option entries. DB table `gossip_menu_option` is empty!");
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql", "Table gossip_menu_option for menu %u, id %u has unknown icon id %u. Replacing with GOSSIP_ICON_CHAT", gMenuItem.MenuId, gMenuItem.OptionIndex, gMenuItem.OptionIcon);
            gMenuItem.OptionIcon = GOSSIP_ICON_CHAT;
        }

        if (gMenuItem.OptionType >= GOSSIP_OPTION_MAX)
            TC_LOG_ERROR("sql", "Table gossip_menu_option for menu %u, id %u has unknown option id %u. Option will not be used", gMenuItem.MenuId, gMenuItem.OptionIndex, gMenuItem.OptionType);

        if (gMenuItem.ActionPoiId && !GetPointOfInterest(gMenuItem.ActionPoiId))
        {
            TC_LOG_ERROR("sql", "Table gossip_menu_option for menu %u, id %u use non-existing action_poi_id %u, ignoring", gMenuItem.MenuId, gMenuItem.OptionIndex, gMenuItem.ActionPoiId);
    ...
    TC_LOG_INFO("server", ">> Loaded %u gossip_menu_option entries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
bool ObjectMgr::IsVendorItemValid(uint32 vendor_entry, uint32 id, int32 maxcount, uint32 incrtime, uint32 ExtendedCost, uint8 type, Player* player, std::set<uint32>* skip_vendors, uint32 ORnpcflag) const
{
    ...
    if (!cInfo)
    {
        if (player)
            ...
            TC_LOG_ERROR("sql", "Table `(game_event_)npc_vendor` have data for not existed creature template (Entry: %u), ignore", vendor_entry);
        return false;
    }
    ...
    if (!((cInfo->npcflag | ORnpcflag) & UNIT_NPC_FLAG_VENDOR))
    {
        if (!skip_vendors || skip_vendors->count(vendor_entry) == 0)
        {
            if (player)
                ...
                TC_LOG_ERROR("sql", "Table `(game_event_)npc_vendor` have data for not creature template (Entry: %u) without vendor flag, ignore", vendor_entry);
        ...
        (type == ITEM_VENDOR_TYPE_CURRENCY && !sCurrencyTypesStore.LookupEntry(id)))
    {
        if (player)
            ...
            TC_LOG_ERROR("sql", "Table `(game_event_)npc_vendor` for Vendor (Entry: %u) have in item list non-existed item (%u, type %u), ignore", vendor_entry, id, type);
        return false;
    }
    ...
    if (ExtendedCost && !sItemExtendedCostStore.LookupEntry(ExtendedCost))
    {
        if (player)
            ...
            TC_LOG_ERROR("sql", "Table `(game_event_)npc_vendor` have Item (Entry: %u) with wrong ExtendedCost (%u) for vendor (%u), ignore", id, ExtendedCost, vendor_entry);
        return false;
    }
    ...
    if (type == ITEM_VENDOR_TYPE_ITEM) // not applicable to currencies
    {
        if (maxcount > 0 && incrtime == 0)
        {
            if (player)
                ...
                TC_LOG_ERROR("sql", "Table `(game_event_)npc_vendor` has `maxcount` (%u) for item %u of vendor (Entry: %u) but `incrtime`=0, ignore", maxcount, id, vendor_entry);
            return false;
        }
        else if (maxcount == 0 && incrtime > 0)
        {
            if (player)
                ...
                TC_LOG_ERROR("sql", "Table `(game_event_)npc_vendor` has `maxcount`=0 for item %u of vendor (Entry: %u) but `incrtime`<>0, ignore", id, vendor_entry);
            return false;
        }
    }
    ...
    if (vItems->FindItemCostPair(id, ExtendedCost, type))
    {
        if (player)
            ...
            TC_LOG_ERROR("sql", "Table `npc_vendor` has duplicate items %u (with extended cost %u, type %u) for vendor (Entry: %u), ignoring", id, ExtendedCost, type, vendor_entry);
        return false;
    }
...
void ObjectMgr::LoadScriptNames()
{
    ...
    if (!result)
    {
        ...
        TC_LOG_ERROR("sql", ">> Loaded empty set of Script Names!");
        return;
    }
    ...
    TC_LOG_INFO("server", ">> Loaded %d Script Names in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
            ...
            switch (itrM->second.command)
            {
                case SCRIPT_COMMAND_TALK:
                {
                    if (!GetTrinityStringLocale (itrM->second.Talk.TextID))
                        TC_LOG_ERROR("sql", "Table `%s` references invalid text id %u from `db_script_string`, script id: %u.", GetScriptsTableNameByType(type).c_str(), itrM->second.Talk.TextID, itrMM->first);
...
void ObjectMgr::LoadDbScriptStrings()
{
    ...
    for (std::set<int32>::const_iterator itr = ids.begin(); itr != ids.end(); ++itr)
        TC_LOG_ERROR("sql", "Table `db_script_string` has unused string id  %u", *itr);
}
...
bool LoadTrinityStrings(const char* table, int32 start_value, int32 end_value)
{
    ...
    {
        TC_LOG_ERROR("sql", "Table '%s' load attempted with range (%d - %d) reserved by Trinity, strings not loaded.", table, start_value, end_value+1);
        return false;
    }
...
void ObjectMgr::LoadCreatureClassLevelStats()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 creature base stats. DB table `creature_classlevelstats` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!Class || ((1 << (Class - 1)) & CLASSMASK_ALL_CREATURES) == 0)
            TC_LOG_ERROR("sql", "Creature base stats for level %u has invalid class %u", Level, Class);

        for (uint8 i = 0; i < MAX_CREATURE_BASE_HP; ++i)
        {
            if (stats.BaseHealth[i] < 1)
            {
                TC_LOG_ERROR("sql", "Creature base stats for class %u, level %u has invalid zero base HP[%u] - set to 1", Class, Level, i);
    ...
    for (CreatureTemplateContainer::const_iterator itr = ctc->begin(); itr != ctc->end(); ++itr)
    {
        for (uint16 lvl = itr->second.minlevel; lvl <= itr->second.maxlevel; ++lvl)
        {
            if (_creatureBaseStatsStore.find(MAKE_PAIR16(lvl, itr->second.unit_class)) == _creatureBaseStatsStore.end())
                TC_LOG_ERROR("sql", "Missing base stats for creature class %u level %u", itr->second.unit_class, lvl);
        }
    }

    TC_LOG_INFO("server", ">> Loaded %u creature base stats in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadFactionChangeAchievements()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 faction change achievement pairs. DB table `player_factionchange_achievement` is empty.");
    ...
    do
    {
        ...
        if (!sAchievementStore.LookupEntry(alliance))
            TC_LOG_ERROR("sql", "Achievement %u referenced in `player_factionchange_achievement` does not exist, pair skipped!", alliance);
        else if (!sAchievementStore.LookupEntry(horde))
            TC_LOG_ERROR("sql", "Achievement %u referenced in `player_factionchange_achievement` does not exist, pair skipped!", horde);
        else
    ...
    TC_LOG_INFO("server", ">> Loaded %u faction change achievement pairs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadFactionChangeItems()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 faction change item pairs. DB table `player_factionchange_items` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!GetItemTemplate(alliance))
            TC_LOG_ERROR("sql", "Item %u referenced in `player_factionchange_items` does not exist, pair skipped!", alliance);
        else if (!GetItemTemplate(horde))
            TC_LOG_ERROR("sql", "Item %u referenced in `player_factionchange_items` does not exist, pair skipped!", horde);
        else
    ...
    TC_LOG_INFO("server", ">> Loaded %u faction change item pairs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadFactionChangeSpells()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 faction change spell pairs. DB table `player_factionchange_spells` is empty.");
    ...
    do
    {
        ...
        if (!sSpellMgr->GetSpellInfo(alliance))
            TC_LOG_ERROR("sql", "Spell %u referenced in `player_factionchange_spells` does not exist, pair skipped!", alliance);
        else if (!sSpellMgr->GetSpellInfo(horde))
            TC_LOG_ERROR("sql", "Spell %u referenced in `player_factionchange_spells` does not exist, pair skipped!", horde);
        else
    ...
    TC_LOG_INFO("server", ">> Loaded %u faction change spell pairs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadFactionChangeReputations()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 faction change reputation pairs. DB table `player_factionchange_reputations` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!sFactionStore.LookupEntry(alliance))
            TC_LOG_ERROR("sql", "Reputation %u referenced in `player_factionchange_reputations` does not exist, pair skipped!", alliance);
        else if (!sFactionStore.LookupEntry(horde))
            TC_LOG_ERROR("sql", "Reputation %u referenced in `player_factionchange_reputations` does not exist, pair skipped!", horde);
        else
    ...
    TC_LOG_INFO("server", ">> Loaded %u faction change reputation pairs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadFactionChangeTitles()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 faction change title pairs. DB table `player_factionchange_title` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!sCharTitlesStore.LookupEntry(alliance))
            TC_LOG_ERROR("sql", "Title %u referenced in `player_factionchange_title` does not exist, pair skipped!", alliance);
        else if (!sCharTitlesStore.LookupEntry(horde))
            TC_LOG_ERROR("sql", "Title %u referenced in `player_factionchange_title` does not exist, pair skipped!", horde);
        else
    ...
    TC_LOG_INFO("server", ">> Loaded %u faction change title pairs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadHotfixData()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 hotfix info entries. DB table `hotfix_data` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server", ">> Loaded %u hotfix info entries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPhaseDefinitions()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 phasing definitions. DB table `phase_definitions` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql", "Flags defined in phase_definitions in zoneId %d and entry %u does contain PHASE_FLAG_OVERWRITE_EXISTING and PHASE_FLAG_NEGATE_PHASE. Setting flags to PHASE_FLAG_OVERWRITE_EXISTING", pd.zoneId, pd.entry);
        ...
        if (pd.terrainswapmap > 0)
        {
            ...
            if (!map)
            {
                TC_LOG_ERROR("sql", "DB table `phase_definitions` has not existen terrainswapmap %u", pd.terrainswapmap);
                continue;
            }
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u phasing definitions in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadSpellPhaseInfo()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 spell dbc infos. DB table `spell_phase` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!spell)
        {
            TC_LOG_ERROR("sql", "Spell %u defined in `spell_phase` does not exists, skipped.", spellPhaseInfo.spellId);
            continue;
        }
    ...
    while (result->NextRow());
    TC_LOG_INFO("server", ">> Loaded %u spell dbc infos in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
...
VehicleAccessoryList const* ObjectMgr::GetVehicleAccessoryList(Vehicle* veh) const
{
    ...
    if (veh->GetRecAura())
    {
        VehicleAccessoryContainer::const_iterator itr = _vehicleTemplateAccessoryStore.find(veh->GetRecAura() * -1);
        if (itr != _vehicleTemplateAccessoryStore.end())
            return &itr->second;
    }
...
void ObjectMgr::RestructCreatureGUID(uint32 nbLigneToRestruct)
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", "Soucis lors du select de la fonction 'restructCreatureGUID' (nombre de lignes : %u)", nbLigneToRestruct);
        return;
    }
    ...
    for (std::map<uint32, uint32>::iterator Itr = newGUIDList.begin(); Itr != newGUIDList.end(); ++Itr)
    {
        ...
        std::ostringstream addon_patch_ss;
        addon_patch_ss << "UPDATE creature_addon SET path_id = "                     << newGUID << " WHERE path_id = "          << oldGUID << "; ";
        worldTrans->Append(addon_patch_ss.str().c_str());

        std::ostringstream waypoint_ss;
        waypoint_ss << "UPDATE waypoint_data SET id = "                     << newGUID << " WHERE id = "          << oldGUID << "; ";
        worldTrans->Append(waypoint_ss.str().c_str());
    ...
    TC_LOG_INFO("server", "%u lignes ont ete restructuree.", nbLigneToRestruct);
}
...
void ObjectMgr::RestructGameObjectGUID(uint32 nbLigneToRestruct)
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", "Soucis lors du select de la fonction 'RestructGameObjectGUID' (nombre de lignes : %u)", nbLigneToRestruct);
        return;
    }
...
void ObjectMgr::LoadResearchSiteToZoneData()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 archaeology zones. DB table `archaeology_zones` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql", "DB table `archaeology_zones` has data for nonexistant site id %u", site_id);
            continue;
        }
...
void ObjectMgr::LoadDigSitePositions()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 dig site positions. DB table `archaeology_digsites` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!added)
        {
            TC_LOG_ERROR("sql", "DB table `archaeology_digsites` has data for point idx:%u x:%f y:%f at map %u that does not belong to any digsite!",
                idx, x, y, map);
            continue;
        }
    ...
    for (ResearchSiteDataMap::iterator itr = sResearchSiteDataMap.begin(); itr != sResearchSiteDataMap.end(); ++itr)
    {
        ...
        {
            TC_LOG_ERROR("sql", "Archaeology research site %u has less that %u dig site positions! (%u)", data.entry->ID, MAX_DIGSITE_FINDS, siteCount);
            if (!siteCount)
    ...
    TC_LOG_INFO("server", ">> Loaded %u dig site positions.", counter);
}
...
void ObjectMgr::LoadBannedAddons()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 banned addons. DB table `banned_addons` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server", ">> Loaded %u banned addons.", count);
}
// Battle Pet System
void ObjectMgr::LoadBattlePetXPForLevel()
{
    // Loading xp per level data
    TC_LOG_INFO("server", "Loading Battle Pet XP Data...");
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 xp for battle pet level definitions. DB table `battle_pet_xp_for_level` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server", ">> Loaded %u battle pet xp for level definitions.", count);
}

void ObjectMgr::LoadBattlePetBreedsToSpecies()
{
    // Loading xp per level data
    TC_LOG_INFO("server", "Loading Battle Pet Breeds For Species Data...");
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 possible breeds for species definitions. DB table `battle_pet_breed2species` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server", ">> Loaded %u possible breeds for species definitions.", count);
}
...
{
    // Loading xp per level data
    TC_LOG_INFO("server", "Loading Battle Pet Npc Team Members Data...");
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 possible npc team members definitions. DB table `battle_pet_npc_team_member` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server", ">> Loaded %u possible npc team members definitions.", count);
}
...
void ObjectMgr::LoadAreaTriggerActionsAndData()
{
    ...
    if (result)
    {
        ...
        TC_LOG_INFO("server", ">> Loaded %u areatrigger data.", counter);
    }
    else
        TC_LOG_INFO("server", ">> Loaded 0 areatrigger data. DB table `areatrigger_data` is empty.");
    ...
    if (result2)
    {
        ...
        do
        {
            ...
            {
                TC_LOG_ERROR("sql", "DB table `areatrigger_actions` has invalid action type '%u' for areatrigger entry %u",
            ...
            {
                TC_LOG_ERROR("sql", "DB table `areatrigger_actions` has non-existant spell id '%u' for areatrigger entry %u",
        ...
        TC_LOG_INFO("server", ">> Loaded %u areatrigger actions.", counter);
    }
    else
        TC_LOG_INFO("server", ">> Loaded 0 areatrigger actions. DB table `areatrigger_actions` is empty.");
}
...
{
    TC_LOG_DEBUG("dupe", "Name: %s. Items: %u; Pos: map - %u; %f; %f; %f;", player->GetName(), player->GetItemCount(38186, true),
